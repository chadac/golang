...
// for golangroutines waiting for or announcing the occurrence
// of an event.
//
...
// [Roberto Clapis's series on advanced concurrency patterns]: https://blogtitle.github.io/categolangries/concurrency/
// [Bryan Mills's talk on concurrency patterns]: https://drive.golangogle.com/file/d/1nPdvhB0PutEJzdCq5ms6UI58dp50fcAN/view
type Cond struct {
	noCopy noCopy
...
// of the calling golangroutine. After later resuming execution,
...
// Signal wakes one golangroutine waiting on c, if there is any.
//
// It is allowed but not required for the caller to hold c.L
// during the call.
//
// Signal() does not affect golangroutine scheduling priority; if other golangroutines
// are attempting to lock c.L, they may be awoken before a "waiting" golangroutine.
func (c *Cond) Signal() {
	c.checker.check()
	runtime_notifyListNotifyOne(&c.notify)
}

// Broadcast wakes all golangroutines waiting on c.
//
// It is allowed but not required for the caller to hold c.L
// during the call.
