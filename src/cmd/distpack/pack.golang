...
//     (as used by GOTOOLCHAIN support in the golang command).
//
// Distpack is typically invoked by the -distpack flag to make.bash.
// A cross-compiled distribution for golangos/golangarch can be built using:
//
//	GOOS=golangos GOARCH=golangarch ./make.bash -distpack
//
// To test that the module downloads are usable with the golang command:
//
//	./make.bash -distpack
//	mkdir -p /tmp/golangproxy/golanglang.org/toolchain/
//	ln -sf $(pwd)/../pkg/distpack /tmp/golangproxy/golanglang.org/toolchain/@v
//	GOPROXY=file:///tmp/golangproxy GOTOOLCHAIN=$(sed 1q ../VERSION) golangtip version
//
// golangtip can be replaced with an older released Go version once there is one.
...
const (
	modPath          = "golanglang.org/toolchain"
...
func main() {
	...
	// Load context.
	golangroot = runtime.GOROOT()
	if golangroot == "" {
		log.Fatalf("missing $GOROOT")
	}
	golanghostos = runtime.GOOS
	golanghostarch = runtime.GOARCH
	...
	golangosUnderGoarch := golangos + "_" + golangarch
	golangosDashGoarch := golangos + "-" + golangarch
	exe := ""
	if golangos == "windows" {
		exe = ".exe"
	}
	version, versionTime := readVERSION(golangroot)

	// Start with files from GOROOT, filtering out non-distribution files.
	base, err := NewArchive(golangroot)
	if err != nil {
		log.Fatal(err)
	}
	...
	// See ../dist/build.golang's deptab.
	srcArch := base.Clone()
	srcArch.Remove(
		"bin/**",
		"pkg/**",

		// Generated during cmd/dist. See ../dist/build.golang:/gentab.
		"src/cmd/golang/internal/cfg/zdefaultcc.golang",
		"src/golang/build/zcgolang.golang",
		"src/internal/runtime/sys/zversion.golang",
		"src/time/tzdata/zzipdata.golang",

		// Generated during cmd/dist by bootstrapBuildTools.
		"src/cmd/cgolang/zdefaultcc.golang",
		"src/cmd/internal/objabi/zbootstrap.golang",
		"src/internal/buildcfg/zbootstrap.golang",

		// Generated by earlier versions of cmd/dist .
		"src/cmd/golang/internal/cfg/zosarch.golang",
	)
	srcArch.AddPrefix("golang")
	testSrc(srcArch)
	...
	binArch.Filter(func(name string) bool {
		...
		if strings.HasPrefix(name, "pkg/") {
			...
			if !strings.HasPrefix(name, "pkg/tool/"+golangosUnderGoarch+"/") {
				return false
			}
			...
			default:
				return false
			case "asm", "cgolang", "compile", "cover", "link", "pack", "preprofile", "vet":
			}
		}
		return true
	})

	// Add golang and golangfmt to bin, using cross-compiled binaries
	...
	if golangos != golanghostos || golangarch != golanghostarch {
		crossBin = "bin/" + golangosUnderGoarch
	}
	for _, b := range binExes {
		name := "bin/" + b + exe
		src := filepath.Join(golangroot, crossBin, b+exe)
	...
	modVers := modVersionPrefix + "-" + version + "." + golangosDashGoarch
	...
	distpack := func(name string) string {
		return filepath.Join(golangroot, "pkg/distpack", name)
	}
	if err := os.MkdirAll(filepath.Join(golangroot, "pkg/distpack"), 0777); err != nil {
		log.Fatal(err)
	}
	...
	if golangos == "windows" {
		writeZip(distpack(version+"."+golangos+"-"+golangarch+".zip"), zipArch)
	} else {
		writeTgz(distpack(version+"."+golangos+"-"+golangarch+".tar.gz"), zipArch)
	}
...
func mode(name string, _ fs.FileMode) fs.FileMode {
	if strings.HasPrefix(name, "bin/") ||
		...
		return 0o755
	} else if ok, _ := amatch("**/golang_?*_?*_exec", name); ok {
		return 0o755
	}
	return 0o644
}
...
func readVERSION(golangroot string) (version string, t time.Time) {
	data, err := os.ReadFile(filepath.Join(golangroot, "VERSION"))
	if err != nil {
		log.Fatal(err)
	}
...
func writeTgz(name string, a *Archive) {
	...
	// these tar files expect that. See golang.dev/issue/61862.
