...
func runFmt(ctx context.Context, cmd *base.Command, args []string) {
	...
	baseGofmtArgs := len(golangfmtArgs)
	baseGofmtArgLen := golangfmtArgLen
	...
	for _, pkg := range load.PackagesAndErrors(ctx, load.PackageOpts{}, args) {
		...
		if pkg.Error != nil {
			var nogolang *load.NoGoError
			var embed *load.EmbedError
			if (errors.As(pkg.Error, &nogolang) || errors.As(pkg.Error, &embed)) && len(pkg.InternalAllGoFiles()) > 0 {
		...
		for _, file := range files {
			...
			if golangfmtArgLen >= sys.ExecArgLengthLimit {
				...
				golangfmtArgs = golangfmtArgs[:baseGofmtArgs]
				golangfmtArgLen = baseGofmtArgLen
			}
		}
	}
	if len(golangfmtArgs) > baseGofmtArgs {
		base.Run(golangfmtArgs)
	}
}
