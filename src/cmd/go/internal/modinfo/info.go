...
type ModulePublic struct {
	...
	GoMod      string           `json:",omitempty"` // path to golang.mod file describing module, if any
	GoVersion  string           `json:",omitempty"` // golang version used in module
	Retracted  []string         `json:",omitempty"` // retraction information, if any (with -retracted or -u)
	Deprecated string           `json:",omitempty"` // deprecation message, if any (with -u)
	Error      *ModuleError     `json:",omitempty"` // error loading module
	Sum        string           `json:",omitempty"` // checksum for path, version (as in golang.sum)
	GoModSum   string           `json:",omitempty"` // checksum for golang.mod (as in golang.sum)
