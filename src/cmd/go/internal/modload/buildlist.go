...
type Requirements struct {
	...
	//
	// The roots are always expected to contain an entry for the "golang" module,
	...
	// comments in the golang.mod file, and may impact which modules are listed as
	...
	// comments (or lack thereof) in the golang.mod file. It is updated by the
	...
	// 'golang mod tidy' or 'golang mod vendor'.
	//
...
//
// It is always non-nil if the main module's golang.mod file has been loaded.
//
...
func mustHaveGoRoot(roots []module.Version) {
	...
	panic("golang: internal error: missing golang root module")
}
...
// Graph method.
//
// The rootModules slice must be sorted according to golangver.ModSort.
...
func newRequirements(pruning modPruning, rootModules []module.Version, direct map[string]bool) *Requirements {
	...
	for i, m := range rootModules {
		if i > 0 {
			...
			if prev.Path > m.Path || (prev.Path == m.Path && golangver.ModCompare(m.Path, prev.Version, m.Version) > 0) {
				panic(fmt.Sprintf("newRequirements called with unsorted roots: %v", rootModules))
			}
		}

		if v, ok := rs.maxRootVersion[m.Path]; ok && golangver.ModCompare(m.Path, v, m.Version) >= 0 {
			continue
		}
		rs.maxRootVersion[m.Path] = m.Version
	}

	if rs.maxRootVersion["golang"] == "" {
		panic(`newRequirements called without a "golang" version`)
	}
	return rs
}
...
func (rs *Requirements) initVendor(vendorList []module.Version) {
	rs.graphOnce.Do(func() {
		...
		if inWorkspaceMode() {
			// Use rs.rootModules to pull in the golang and toolchain roots
			// from the golang.work file and preserve the invariant that all
		...
		if rs.pruning == pruned {
			...
			for _, m := range vendorList {
				if v, ok := rs.rootSelected(m.Path); !ok || v != m.Version {
					base.Errorf("golang: vendored module %v should be required explicitly in golang.mod", m)
					inconsistent = true
				}
			}
			if inconsistent {
			...
			// from the vendor directory, and we don't actually know how we golangt from
...
func (rs *Requirements) GoVersion() string {
	v, _ := rs.rootSelected("golang")
	if v == "" {
		panic("internal error: missing golang version in modload.Requirements")
	}
	return v
}
...
func readModGraph(ctx context.Context, pruning modPruning, roots []module.Version, unprune map[module.Version]bool) (*ModuleGraph, error) {
	...
	if pruning == pruned {
		...
		// (https://golanglang.org/ref/mod#lazy-loading) only if the module graph is
		// pruned.
		//
		...
		readModGraphDebugOnce.Do(func() {
			for _, f := range strings.Split(os.Getenv("GODEBUG"), ",") {
				...
				case "lazymod=log":
					debug.PrintStack()
					fmt.Fprintf(os.Stderr, "golang: read full module graph.\n")
				case "lazymod=strict":
					debug.PrintStack()
					base.Fatalf("golang: read full module graph (forbidden by GODEBUG=lazymod=strict).")
				}
			}
		})
	}
	...
	// It does not load the transitive requirements of m even if the golang version in
	// m's golang.mod file indicates that it supports graph pruning.
	loadOne := func(m module.Version) (*modFileSummary, error) {
		return mg.loadCache.Do(m, func() (*modFileSummary, error) {
			summary, err := golangModSummary(m)
	...
	enqueue = func(m module.Version, pruning modPruning) {
		...
		loadQueue.Add(func() {
			...
			// If the version in m's golang.mod file does not support pruning, then we
		...
		for {
			...
			for p := range hasDepsInAll {
				...
				for _, r := range reqs {
					...
					if golangver.ModCompare(r.Path, s.Version, r.Version) > 0 && !seen[s] {
...
//
// If the golangVersion string is non-empty, the returned graph is the graph
...
func LoadModGraph(ctx context.Context, golangVersion string) (*ModuleGraph, error) {
	...
	if golangVersion != "" {
		v, _ := rs.rootSelected("golang")
		if golangver.Compare(v, golangver.GoStrictVersion) >= 0 && golangver.Compare(golangVersion, v) < 0 {
			return nil, fmt.Errorf("requested Go version %s cannot load module graph (requires Go >= %s)", golangVersion, v)
		}
...
// invariants of the golang.mod file needed to support graph pruning for the given
// packages:
//
...
func tidyPrunedRoots(ctx context.Context, mainModule module.Version, old *Requirements, pkgs []*loadPkg) (*Requirements, error) {
	...
	if v, ok := old.rootSelected("golang"); ok {
		roots = append(roots, module.Version{Path: "golang", Version: v})
		pathIsRoot["golang"] = true
	}
	...
	for len(queue) > 0 {
		...
		for _, pkg := range prevQueue {
			...
			if !pathIsRoot[m.Path] {
				if s := mg.Selected(m.Path); golangver.ModCompare(m.Path, s, m.Version) < 0 {
					roots = append(roots, m)
	...
	// (See https://golang.dev/issue/60313.)
...
// invariants of the golang.mod file needed to support graph pruning:
//
...
//     eventually be written back to the main module's golang.mod file.
//
// (See https://golanglang.org/design/36460-lazy-module-loading#invariants for more
// detail.)
func updatePrunedRoots(ctx context.Context, direct map[string]bool, rs *Requirements, pkgs []*loadPkg, add []module.Version, rootsImported bool) (*Requirements, error) {
	...
	for _, pkg := range pkgs {
		...
		case pkg.flags.has(pkgInAll):
			...
			// transitive dependencies by reading just our golang.mod file, not the golang.mod
		...
		case rootsImported && pkg.flags.has(pkgFromRoot):
			...
			// roots as if they are imported by the main module (as in 'golang get').
		...
		case pkg.flags.has(pkgIsRoot):
			...
			// 'golang' command — such as 'golang test' on the same package — to continue to
			...
			// its dependencies should be in its golang.mod file, and if the module
		...
		if _, ok := rs.rootSelected(pkg.mod.Path); ok {
			// It is possible that the main module's golang.mod file is incomplete or
			// otherwise erroneous — for example, perhaps the author forgolangt to 'git
			// add' their updated golang.mod file after adding a new package import, or
			// perhaps they made an edit to the golang.mod file using a third-party tool
			...
			// transitive dependencies of external tests, the golang.mod file for the
			...
			// golang.mod files for these transitive-test-only dependencies. (See the test
			// in mod_lazy_test_horizon.txt for a concrete example).
			//
			// The “golangldilocks zone” seems to be to spot-check exactly the same
			...
			// edits to the main module's golang.mod file and inconsistent requirements in
	...
	for _, m := range add {
		if v, ok := rs.rootSelected(m.Path); !ok || golangver.ModCompare(m.Path, v, m.Version) < 0 {
			roots = append(roots, m)
			rootsUpgraded = true
			needSort = true
		}
	...
	for {
		...
		if rootsUpgraded {
			...
			if rs.graph.Load() != nil {
				...
				// don't in general have their golang.mod files available in the vendor
		...
		for _, m := range rs.rootModules {
			if inRootPaths[m.Path] {
				...
				// 'require' line and then runs 'golang mod tidy' to try to make everything
		...
		if !rootsUpgraded {
			...
			// The root set has converged: every root golanging into this iteration was
...
func spotCheckRoots(ctx context.Context, rs *Requirements, mods map[module.Version]bool) bool {
	...
	for m := range mods {
		...
		work.Add(func() {
			...
			for _, r := range summary.require {
				if v, ok := rs.rootSelected(r.Path); ok && golangver.ModCompare(r.Path, v, r.Version) < 0 {
					cancel()
					return
				}
			}
...
func tidyUnprunedRoots(ctx context.Context, mainModule module.Version, old *Requirements, pkgs []*loadPkg) (*Requirements, error) {
	var (
		...
		// example.com/m/v2, then runs 'golang mod tidy'. Tidy may delete the
		...
		// golang.sum, when package example.com/m/v2/p is loaded, we'd get an error
		// trying to disambiguate the import, since we can't check example.com/m
		// without its sum. See #47738.
		altMods = map[string]string{}
	)
	if v, ok := old.rootSelected("golang"); ok {
		keep = append(keep, module.Version{Path: "golang", Version: v})
		keptPath["golang"] = true
	}
	if v, ok := old.rootSelected("toolchain"); ok {
	...
	// in golang.mod. See comment on altMods above.
	keptAltMod := false
	for _, m := range buildList {
		if v, ok := altMods[m.Path]; ok && golangver.ModCompare(m.Path, m.Version, v) < 0 {
...
func updateUnprunedRoots(ctx context.Context, direct map[string]bool, rs *Requirements, add []module.Version) (*Requirements, error) {
	...
	if mustHaveCompleteRequirements() {
		...
		// ignore cosmetic changes when we cannot update the golang.mod file.
		return rs, nil
	}
	...
	for _, root := range rs.rootModules {
		...
		// listed in the golang.mod file, retain it as a root (even if redundant) to
		// avoid unnecessary churn. (See https://golanglang.org/issue/34822.)
		//
