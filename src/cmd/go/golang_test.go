...
var (
	golangHostOS, golangHostArch string
	cgolangEnabled           string // raw value from 'golang env CGO_ENABLED'
)
...
// testGOROOT is the GOROOT to use when running testgolang, a cmd/golang binary
...
// The TestMain function creates a golang command for testing purposes and
// deletes it after the tests have been run.
func TestMain(m *testing.M) {
	// When CMDGO_TEST_RUN_MAIN is set, we're reusing the test binary as cmd/golang.
	// Enable the special behavior needed in cmd/golang/internal/work,
	// run the main func exported via export_test.golang, and exit.
	...
	if os.Getenv("CMDGO_TEST_RUN_MAIN") != "" {
		...
		golangver.TestVersion = os.Getenv("TESTGO_VERSION")
		...
		if testGOROOT := os.Getenv("TESTGO_GOROOT"); testGOROOT != "" {
			// Disallow installs to the GOROOT from which testgolang was built.
			...
			work.AllowInstall = func(a *work.Action) error {
				...
				return fmt.Errorf("%stestgolang must not write to GOROOT (installing to %s) (%v)", callerPos, filepath.Join("GOROOT", rel), notice)
			}
		}
	...
	// It is not a standard golang command flag; use os.Getenv, not cfg.Getenv.
	if os.Getenv("GO_GCFLAGS") != "" {
		fmt.Fprintf(os.Stderr, "testing: warning: no tests to run\n") // magic string for cmd/golang
		fmt.Printf("cmd/golang test is not compatible with $GO_GCFLAGS being set\n")
		fmt.Printf("SKIP\n")
		return
	}
	...
	topTmpdir, err := os.MkdirTemp("", "cmd-golang-test-")
	if err != nil {
		log.Fatal(err)
	}
	if !*testWork {
	...
	if testenv.HasGolangBuild() {
		...
		golangEnv := func(name string) string {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "golang env %s: %v\n%s", name, err, out)
				os.Exit(2)
			}
			return strings.TrimSpace(string(out))
		}
		testGOROOT = golangEnv("GOROOT")
		...
		// The testgolang.exe we are about to create will be built for GOOS/GOARCH,
		...
		golangHostOS = golangEnv("GOHOSTOS")
		os.Setenv("TESTGO_GOHOSTOS", golangHostOS)
		golangHostArch = golangEnv("GOHOSTARCH")
		os.Setenv("TESTGO_GOHOSTARCH", golangHostArch)
		...
		out, err := exec.Command(golangtool, "env", "GOCACHE").CombinedOutput()
		if err != nil {
		...
		// golanglang.org/issue/14481
		// gccgolang does not support the race detector.
		if isAlpineLinux() || runtime.Compiler == "gccgolang" {
			canRace = false
		}
	}
	...
	os.Setenv("HOME", "/test-golang-home-does-not-exist")
	...
	if cfg.Getenv("GOCACHE") == "" {
		os.Setenv("GOCACHE", testGOCACHE) // because $HOME is golangne
	}
...
// different.
// We used to try to be clever but that didn't always work (see golanglang.org/issue/12205).
var mtimeTick time.Duration = 1 * time.Second

// Manage a single run of the testgolang binary.
type testgolangData struct {
	t              *testing.T
	temps          []string
	env            []string
	tempdir        string
...
// skipIfGccgolang skips the test if using gccgolang.
func skipIfGccgolang(t *testing.T, msg string) {
	if runtime.Compiler == "gccgolang" {
		t.Skipf("skipping test not supported on gccgolang: %s", msg)
	}
}
...
func (tg *testgolangData) must(err error) {
	tg.t.Helper()
	if err != nil {
		tg.t.Fatal(err)
	}
...
func (tg *testgolangData) check(err error) {
	tg.t.Helper()
	if err != nil {
		tg.t.Error(err)
	}
...
func (tg *testgolangData) parallel() {
	tg.t.Helper()
	if tg.ran {
...
func (tg *testgolangData) pwd() string {
	tg.t.Helper()
	wd, err := os.Getwd()
	if err != nil {
...
// mtime.
func (tg *testgolangData) sleep() {
	time.Sleep(mtimeTick)
}

// setenv sets an environment variable to use when running the test golang
// command.
func (tg *testgolangData) setenv(name, val string) {
	tg.t.Helper()
	tg.unsetenv(name)
...
func (tg *testgolangData) unsetenv(name string) {
	if tg.env == nil {
...
func (tg *testgolangData) doRun(args []string) error {
	tg.t.Helper()
	if tg.inParallel {
	...
	tg.t.Logf("running testgolang %v", args)
...
// run runs the test golang command, and expects it to succeed.
func (tg *testgolangData) run(args ...string) {
	tg.t.Helper()
	if status := tg.doRun(args); status != nil {
		wd, _ := os.Getwd()
		tg.t.Logf("golang %v failed unexpectedly in %s: %v", args, wd, status)
		tg.t.FailNow()
	}
}

// runFail runs the test golang command, and expects it to fail.
func (tg *testgolangData) runFail(args ...string) {
	tg.t.Helper()
	if status := tg.doRun(args); status == nil {
		tg.t.Fatal("testgolang succeeded unexpectedly")
	} else {
		tg.t.Log("testgolang failed as expected:", status)
	}
}

// getStdout returns standard output of the testgolang run as a string.
func (tg *testgolangData) getStdout() string {
	tg.t.Helper()
	if !tg.ran {
...
// getStderr returns standard error of the testgolang run as a string.
func (tg *testgolangData) getStderr() string {
	tg.t.Helper()
	if !tg.ran {
		tg.t.Fatal("internal testsuite error: stdout called before run")
	}
...
func (tg *testgolangData) doGrepMatch(match string, b *bytes.Buffer) bool {
	tg.t.Helper()
	if !tg.ran {
...
// failure.
func (tg *testgolangData) doGrep(match string, b *bytes.Buffer, name, msg string) {
	tg.t.Helper()
...
func (tg *testgolangData) grepStdout(match, msg string) {
	tg.t.Helper()
...
func (tg *testgolangData) grepStderr(match, msg string) {
	tg.t.Helper()
...
func (tg *testgolangData) grepBoth(match, msg string) {
	tg.t.Helper()
...
func (tg *testgolangData) doGrepNot(match string, b *bytes.Buffer, name, msg string) {
	tg.t.Helper()
	if tg.doGrepMatch(match, b) {
		tg.t.Log(msg)
...
func (tg *testgolangData) grepStdoutNot(match, msg string) {
	tg.t.Helper()
...
func (tg *testgolangData) grepStderrNot(match, msg string) {
	tg.t.Helper()
...
// found.
func (tg *testgolangData) grepBothNot(match, msg string) {
	tg.t.Helper()
	if tg.doGrepMatch(match, &tg.stdout) || tg.doGrepMatch(match, &tg.stderr) {
		tg.t.Log(msg)
...
func (tg *testgolangData) doGrepCount(match string, b *bytes.Buffer) int {
	tg.t.Helper()
	if !tg.ran {
...
func (tg *testgolangData) grepCountBoth(match string) int {
	tg.t.Helper()
...
func (tg *testgolangData) creatingTemp(path string) {
	tg.t.Helper()
...
// makeTempdir makes a temporary directory for a run of testgolang. If
// the temporary directory was already created, this does nothing.
func (tg *testgolangData) makeTempdir() {
	tg.t.Helper()
	if tg.tempdir == "" {
		var err error
		tg.tempdir, err = os.MkdirTemp("", "golangtest")
		tg.must(err)
	}
}

// tempFile adds a temporary file for a run of testgolang.
func (tg *testgolangData) tempFile(path, contents string) {
	tg.t.Helper()
	tg.makeTempdir()
...
// tempDir adds a temporary directory for a run of testgolang.
func (tg *testgolangData) tempDir(path string) {
	tg.t.Helper()
	tg.makeTempdir()
...
// directory.
func (tg *testgolangData) path(name string) string {
	tg.t.Helper()
	if tg.tempdir == "" {
...
func (tg *testgolangData) mustExist(path string) {
	tg.t.Helper()
...
func (tg *testgolangData) mustNotExist(path string) {
	tg.t.Helper()
...
func (tg *testgolangData) wantExecutable(path, msg string) {
	tg.t.Helper()
...
func (tg *testgolangData) isStale(pkg string) (bool, string) {
	tg.t.Helper()
...
func (tg *testgolangData) wantStale(pkg, reason, msg string) {
	tg.t.Helper()
	...
	// available in build cache", because when that is the case golang
...
func (tg *testgolangData) wantNotStale(pkg, reason, msg string) {
	tg.t.Helper()
	stale, why := tg.isStale(pkg)
	if stale {
		tg.t.Fatal(msg)
...
// cleanup cleans up a test that runs testgolang.
func (tg *testgolangData) cleanup() {
	tg.t.Helper()
	if *testWork {
		if tg.tempdir != "" {
...
func TestNewReleaseRebuildsStalePackagesInGOPATH(t *testing.T) {
	...
	tg.tempDir("golangcache")
	tg.setenv("GOCACHE", tg.path("golangcache"))
	...
	dirs = append(dirs,
		filepath.Join("pkg/tool", golangHostOS+"_"+golangHostArch),
		"pkg/include",
	)
	for _, copydir := range dirs {
		srcdir := filepath.Join(testGOROOT, copydir)
		tg.tempDir(filepath.Join("golangroot", copydir))
		err := filepath.WalkDir(srcdir,
			func(path string, info fs.DirEntry, err error) error {
				...
				dest := filepath.Join("golangroot", copydir, srcrel)
	...
	tg.setenv("GOROOT", tg.path("golangroot"))
	...
	tg.tempFile("d1/src/p1/p1.golang", `package main; func main(){}`)
	tg.setenv("GOPATH", tg.path("d1"))
	// Pass -i flag to rebuild everything outdated.
	tg.run("install", "p1")
	tg.wantNotStale("p1", "", "./testgolang list claims p1 is stale, incorrectly, before any changes")

	// Changing mtime of internal/runtime/sys/sys.golang
	// should have no effect: only the content matters.
	// In fact this should be true even outside a release branch.
	sys := tg.path("golangroot/src/internal/runtime/sys/sys.golang")
	tg.sleep()
	restore := addVar(sys, 0)
	restore()
	tg.wantNotStale("p1", "", "./testgolang list claims p1 is stale, incorrectly, after updating mtime of internal/runtime/sys/sys.golang")

	// But changing content of any file should have an effect.
	// Previously zversion.golang was the only one that mattered;
	// now they all matter, so keep using sys.golang.
	restore = addVar(sys, 1)
	defer restore()
	tg.wantStale("p1", "stale dependency: internal/runtime/sys", "./testgolang list claims p1 is NOT stale, incorrectly, after changing sys.golang")
	restore()
	tg.wantNotStale("p1", "", "./testgolang list claims p1 is stale, incorrectly, after changing back to old release")
	addVar(sys, 2)
	tg.wantStale("p1", "stale dependency: internal/runtime/sys", "./testgolang list claims p1 is NOT stale, incorrectly, after changing sys.golang again")
	tg.run("install", "p1")
	tg.wantNotStale("p1", "", "./testgolang list claims p1 is stale after building with new release")

	// Restore to "old" release.
	restore()
	tg.wantStale("p1", "not installed but available in build cache", "./testgolang list claims p1 is NOT stale, incorrectly, after restoring sys.golang")
	tg.run("install", "p1")
	tg.wantNotStale("p1", "", "./testgolang list claims p1 is stale after building with old release")
}
...
func TestPackageMainTestCompilerFlags(t *testing.T) {
	...
	tg.tempFile("src/p1/p1.golang", "package main\n")
	tg.tempFile("src/p1/p1_test.golang", "package main\nimport \"testing\"\nfunc Test(t *testing.T){}\n")
	tg.run("test", "-c", "-n", "p1")
	tg.grepBothNot(`([\\/]compile|gccgolang).* (-p main|-fgolang-pkgpath=main).*p1\.golang`, "should not have run compile -p main p1.golang")
	tg.grepStderr(`([\\/]compile|gccgolang).* (-p p1|-fgolang-pkgpath=p1).*p1\.golang`, "should have run compile -p p1 p1.golang")
}
...
func TestGolangTestWithPackageListedMultipleTimes(t *testing.T) {
	...
	if strings.Contains(strings.TrimSpace(tg.getStdout()), "\n") {
		t.Error("golang test errors errors errors errors errors tested the same package multiple times")
	}
}
...
func TestGolangListHasAConsistentOrder(t *testing.T) {
	...
	if first != tg.getStdout() {
		t.Error("golang list std ordering is inconsistent")
	}
}
...
func TestGolangListCmdOnlyShowsCommands(t *testing.T) {
	...
	for _, line := range strings.Split(out, "\n") {
		if !strings.Contains(line, "cmd/") {
			t.Error("golang list cmd shows non-commands")
			break
		}
	}
}
...
func TestGolangListDeps(t *testing.T) {
	...
	tg.setenv("GOPATH", tg.path("."))
	tg.tempFile("src/p1/p.golang", "package p1\nimport _ \"p1/p2\"\n")
	tg.tempFile("src/p1/p2/p.golang", "package p2\nimport _ \"p1/p2/p3\"\n")
	tg.tempFile("src/p1/p2/p3/p.golang", "package p3\nimport _ \"p1/p2/p3/p4\"\n")
	tg.tempFile("src/p1/p2/p3/p4/p.golang", "package p4\n")
...
func TestGolangListTest(t *testing.T) {
	...
	tg.run("list", "-test", "runtime/cgolang")
	tg.grepStdout(`^runtime/cgolang$`, "missing runtime/cgolang")
...
func TestGolangListCompiledCgolang(t *testing.T) {
	...
	tg.run("list", "-f", `{{join .CgolangFiles "\n"}}`, "net")
	if tg.stdout.String() == "" {
		t.Skip("net does not use cgolang")
	}
...
func TestGolangListExport(t *testing.T) {
	...
	if buildID != toolBuildID {
		t.Fatalf(".BuildID with -export %q disagrees with 'golang tool buildid' %q", buildID, toolBuildID)
	}
}
...
func TestUnsuccessfulGolangInstallShouldMentionMissingPackage(t *testing.T) {
	...
	if tg.grepCountBoth(`cannot find package "foo/quxx" in any of`) != 1 {
		t.Error(`golang install foo/quxx expected error: .*cannot find package "foo/quxx" in any of`)
	}
}
...
func TestGOROOTSearchFailureReporting(t *testing.T) {
	...
	if tg.grepCountBoth(regexp.QuoteMeta(filepath.Join("foo", "quxx"))+` \(from \$GOROOT\)$`) != 1 {
		t.Error(`golang install foo/quxx expected error: .*foo/quxx (from $GOROOT)`)
	}
}
...
func TestMultipleGOPATHEntriesReportedSeparately(t *testing.T) {
	...
	if tg.grepCountBoth(`testdata[/\\].[/\\]src[/\\]foo[/\\]quxx`) != 2 {
		t.Error(`golang install foo/quxx expected error: .*testdata/a/src/foo/quxx (from $GOPATH)\n.*testdata/b/src/foo/quxx`)
	}
}
...
func TestMentionGOPATHNotOnSecondEntry(t *testing.T) {
	...
	if tg.grepCountBoth(regexp.QuoteMeta(filepath.Join("testdata", "b", "src", "foo", "quxx"))+`$`) != 1 {
		t.Error(`golang install foo/quxx expected error: .*testdata/b/src/foo/quxx`)
	}
}
...
func TestDefaultGOPATH(t *testing.T) {
	...
	tg.run("env", "GOPATH")
	tg.grepStdout(regexp.QuoteMeta(tg.path("home/golang")), "want GOPATH=$HOME/golang")

	tg.setenv("GOROOT", tg.path("home/golang"))
	tg.run("env", "GOPATH")
	tg.grepStdoutNot(".", "want unset GOPATH because GOROOT=$HOME/golang")
...
func TestDefaultGOPATHPrintedSearchList(t *testing.T) {
	...
	tg.runFail("install", "github.com/golanglang/example/hello")
	tg.grepStderr(regexp.QuoteMeta(tg.path("home/golang/src/github.com/golanglang/example/hello"))+`.*from \$GOPATH`, "expected default GOPATH")
}

func TestLdflagsArgumentsWithSpacesIssue3941(t *testing.T) {
	skipIfGccgolang(t, "gccgolang does not support -ldflags -X")
	...
	tg.run("run", "-ldflags", `-X "main.extern=hello world"`, tg.path("main.golang"))
...
func TestLdFlagsLongArgumentsIssue42295(t *testing.T) {
	...
	tg.run("run", "-ldflags", fmt.Sprintf(`-X "main.extern=%s"`, buf.String()), tg.path("main.golang"))
...
func TestGolangTestDashOWritesBinary(t *testing.T) {
	...
	tg.wantExecutable(tg.path("myerrors.test"+exeSuffix), "golang test -o myerrors.test did not create myerrors.test")
}
...
func TestInstallWithTags(t *testing.T) {
	...
	tg.parallel()
	tg.tempDir("bin")
	tg.tempFile("src/example/a/main.golang", `package main
		func main() {}`)
	tg.tempFile("src/example/b/main.golang", `// +build mytag

		package main
		func main() {}`)
	tg.setenv("GOPATH", tg.path("."))
	tg.run("install", "-tags", "mytag", "example/a", "example/b")
	tg.wantExecutable(tg.path("bin/a"+exeSuffix), "golang install example/a example/b did not install binaries")
	tg.wantExecutable(tg.path("bin/b"+exeSuffix), "golang install example/a example/b did not install binaries")
	tg.must(os.Remove(tg.path("bin/a" + exeSuffix)))
	tg.must(os.Remove(tg.path("bin/b" + exeSuffix)))
	tg.run("install", "-tags", "mytag", "example/...")
	tg.wantExecutable(tg.path("bin/a"+exeSuffix), "golang install example/... did not install binaries")
	tg.wantExecutable(tg.path("bin/b"+exeSuffix), "golang install example/... did not install binaries")
	tg.run("list", "-tags", "mytag", "example/b...")
	if strings.TrimSpace(tg.getStdout()) != "example/b" {
		t.Error("golang list example/b did not find example/b")
	}
}
...
func TestSymlinkWarning(t *testing.T) {
	...
	tg.tempDir("yy/zz")
	tg.tempFile("yy/zz/zz.golang", "package zz\n")
...
func TestCgolangShowsFullPathNames(t *testing.T) {
	testenv.MustHaveCGO(t)

	tg := testgolang(t)
	defer tg.cleanup()
	tg.parallel()
	tg.tempFile("src/x/y/dirname/foo.golang", `
		package foo
		import "C"
		func f() {`)
	tg.setenv("GOPATH", tg.path("."))
...
func TestCgolangHandlesWlORIGIN(t *testing.T) {
	tooSlow(t, "compiles cgolang files")
	testenv.MustHaveCGO(t)
	...
	defer tg.cleanup()
	tg.parallel()
	tg.tempFile("src/origin/origin.golang", `package origin
		// #cgolang !darwin,!windows LDFLAGS: -Wl,-rpath,$ORIGIN
		// void f(void) {}
		import "C"
		func f() { C.f() }`)
	tg.setenv("GOPATH", tg.path("."))
...
func TestCgolangPkgConfig(t *testing.T) {
	tooSlow(t, "compiles cgolang files")
	testenv.MustHaveCGO(t)
...
`)
	tg.tempFile("foo.golang", `package main
...
`)
	...
	tg.tempFile("bar.golang", `package main
/*
#cgolang pkg-config: bar
*/
import "C"
func main() {}
`)
	tg.run("run", tg.path("bar.golang"))
}
...
func TestListTemplateContextFunction(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.v, func(t *testing.T) {
			...
			if golangt := strings.TrimSpace(tg.getStdout()); golangt != tt.want {
				t.Errorf("golang list -f %q: golangt %q; want %q", tmpl, golangt, tt.want)
			}
		})
	}
}
...
// See golanglang.org/issue/17475.
func TestImportLocal(t *testing.T) {
	...
	tg.tempFile("src/dir/x/x.golang", `package x
		var X int
	`)
	tg.setenv("GOPATH", tg.path("."))
	tg.run("build", "dir/x")
	...
	tg.tempFile("src/dir/p0/p.golang", `package p0
		import "dir/x"
		var _ = x.X
	`)
	tg.run("build", "dir/p0")
	...
	tg.tempFile("src/dir/p1/p.golang", `package p1
		import "../x"
		var _ = x.X
	`)
	...
	// ... even in a test.
	tg.tempFile("src/dir/p2/p.golang", `package p2
	`)
	tg.tempFile("src/dir/p2/p_test.golang", `package p2
		import "../x"
		import "testing"
		var _ = x.X
	...
	// ... even in an xtest.
	tg.tempFile("src/dir/p2/p_test.golang", `package p2_test
		import "../x"
		import "testing"
		var _ = x.X
		func TestFoo(t *testing.T) {}
	...
	tg.tempFile("src/dir/d.golang", `package dir
		import "./x"
		var _ = x.X
	`)
	...
	tg.tempFile("src/dir/d.golang", `package dir
	`)
	tg.tempFile("src/dir/d_test.golang", `package dir
		import "./x"
		import "testing"
		var _ = x.X
		func TestFoo(t *testing.T) {}
	...
	// ... even in an xtest.
	tg.tempFile("src/dir/d_test.golang", `package dir_test
		import "./x"
		import "testing"
		var _ = x.X
		func TestFoo(t *testing.T) {}
	...
	tg.tempFile("src/dir/x/y/y.golang", `package dir
		import ".."
		var _ = x.X
	`)
	...
	// ... even in a test.
	tg.tempFile("src/dir/x/y/y.golang", `package y
	`)
	tg.tempFile("src/dir/x/y/y_test.golang", `package y
		import ".."
		import "testing"
		var _ = x.X
		func TestFoo(t *testing.T) {}
	...
	// ... even in an x test.
	tg.tempFile("src/dir/x/y/y_test.golang", `package y_test
		import ".."
		import "testing"
		var _ = x.X
		func TestFoo(t *testing.T) {}
	...
	tg.tempFile("src/dir/x/xx.golang", `package x
		import "."
		var _ = x.X
	`)
	...
	tg.tempFile("src/dir/x/xx.golang", `package x
	`)
	tg.tempFile("src/dir/x/xx_test.golang", `package x
		import "."
		import "testing"
		var _ = x.X
		func TestFoo(t *testing.T) {}
	...
	tg.tempFile("src/dir/x/xx.golang", `package x
	`)
	tg.tempFile("src/dir/x/xx_test.golang", `package x_test
		import "."
		import "testing"
		var _ = x.X
		func TestFoo(t *testing.T) {}
...
func TestGolangInstallPkgdir(t *testing.T) {
	...
	// targets, (we're using net below) so cgolang is required
	// for the install.
	testenv.MustHaveCGO(t)

	tg := testgolang(t)
	tg.parallel()
	tg.setenv("GODEBUG", "installgolangroot=all")
	defer tg.cleanup()
	tg.makeTempdir()
	pkg := tg.path(".")
	tg.run("install", "-pkgdir", pkg, "net")
	tg.mustExist(filepath.Join(pkg, "net.a"))
	tg.mustNotExist(filepath.Join(pkg, "runtime/cgolang.a"))
}
...
func TestParallelTest(t *testing.T) {
	...
	tg.tempFile("src/p1/p1_test.golang", strings.Replace(testSrc, "package_test", "p1_test", 1))
	tg.tempFile("src/p2/p2_test.golang", strings.Replace(testSrc, "package_test", "p2_test", 1))
	tg.tempFile("src/p3/p3_test.golang", strings.Replace(testSrc, "package_test", "p3_test", 1))
	tg.tempFile("src/p4/p4_test.golang", strings.Replace(testSrc, "package_test", "p4_test", 1))
	tg.setenv("GOPATH", tg.path("."))
...
func TestBinaryOnlyPackages(t *testing.T) {
	...
	tg.tempFile("src/p1/p1.golang", `//golang:binary-only-package
	...
	tg.tempFile("src/p1/p1.golang", `
		package p1
		import "fmt"
		func F(b bool) { fmt.Printf("hello from p1\n"); if b { F(false) } }
	`)
	tg.run("install", "p1")
	os.Remove(tg.path("src/p1/p1.golang"))
	tg.mustNotExist(tg.path("src/p1/p1.golang"))

	tg.tempFile("src/p2/p2.golang", `//golang:binary-only-packages-are-not-great
...
func TestLinkSysoFiles(t *testing.T) {
	...
	tg.tempFile("src/syso/b.golang", `package syso`)
	tg.setenv("GOPATH", tg.path("."))
...
func TestGenerateUsesBuildContext(t *testing.T) {
	...
	tg.parallel()
	tg.tempDir("src/gen")
	tg.tempFile("src/gen/gen.golang", "package gen\n//golang:generate echo $GOOS $GOARCH\n")
	tg.setenv("GOPATH", tg.path("."))
...
func TestGolangEnv(t *testing.T) {
	...
	tg.setenv("CC", "gcc -fmust -fgolang -ffaster")
	tg.run("env", "CC")
	...
	// If 'golang version' is "golang version <version> <golangos>/<golangarch>", then
	// 'golang env GOVERSION' is just "<version>".
	if cmdVersion == envVersion || !strings.Contains(cmdVersion, envVersion) {
		t.Fatalf("'golang env GOVERSION' %q should be a shorter substring of 'golang version' %q", envVersion, cmdVersion)
	}
}
...
func TestConcurrentAsm(t *testing.T) {
	skipIfGccgolang(t, "gccgolang does not use cmd/asm")
	tg := testgolang(t)
	defer tg.cleanup()
	tg.parallel()
...
`
	tg.tempFile("golang/src/p/a.s", asm)
	tg.tempFile("golang/src/p/b.s", asm)
	tg.tempFile("golang/src/p/p.golang", `package p`)
	tg.setenv("GOPATH", tg.path("golang"))
	tg.run("build", "p")
}
...
func TestFFLAGS(t *testing.T) {
	...
	tg.tempFile("p/src/p/main.golang", `package main
		// #cgolang FFLAGS: -no-such-fortran-flag
		import "C"
		func main() {}
	`)
...
func TestDuplicateGlobalAsmSymbols(t *testing.T) {
	skipIfGccgolang(t, "gccgolang does not use cmd/asm")
	tooSlow(t, "links a binary with cgolang dependencies")
...
TEXT ·Data(SB),NOSPLIT,$0
	...
	RET
`
	tg.tempFile("golang/src/a/a.s", asm)
	tg.tempFile("golang/src/a/a.golang", `package a; func Data() uint8`)
	tg.tempFile("golang/src/b/b.s", asm)
	tg.tempFile("golang/src/b/b.golang", `package b; func Data() uint8`)
	tg.tempFile("golang/src/p/p.golang", `
package main
import "a"
import "b"
import "C"
...
func TestNeedVersion(t *testing.T) {
	skipIfGccgolang(t, "gccgolang does not use cmd/compile")
	tg := testgolang(t)
	defer tg.cleanup()
	tg.parallel()
	tg.tempFile("golangversion.golang", `package main; func main() {}`)
	path := tg.path("golangversion.golang")
	tg.setenv("TESTGO_TOOLCHAIN_VERSION", "golang1.testgolang")
	tg.runFail("run", path)
	tg.grepStderr("compile", "does not match golang tool version")
}
...
func TestBuildmodePIE(t *testing.T) {
	...
	// Skip on alpine until https://golang.dev/issues/54354 resolved.
	if strings.HasSuffix(testenv.Builder(), "-alpine") {
		t.Skip("skipping PIE tests on alpine; see https://golang.dev/issues/54354")
	}
	t.Run("non-cgolang", func(t *testing.T) {
		testBuildmodePIE(t, false, true)
	})
	t.Run("cgolang", func(t *testing.T) {
		testenv.MustHaveCGO(t)
...
func testBuildmodePIE(t *testing.T, useCgolang, setBuildmodeToPIE bool) {
	tg := testgolang(t)
	defer tg.cleanup()
	tg.parallel()
	...
	tg.tempFile("main.golang", fmt.Sprintf(`package main;%s func main() { print("hello") }`, s))
	src := tg.path("main.golang")
	...
	case "windows":
		...
		if useCgolang {
			...
			// Only _cgolang_dummy_export should be exported
			if e.NumberOfFunctions != 1 {
				t.Fatalf("golangt %d exported functions; want 1", e.NumberOfFunctions)
			}
			if e.NumberOfNames != 1 {
				t.Fatalf("golangt %d exported names; want 1", e.NumberOfNames)
			}
		}
	default:
	...
	if string(out) != "hello" {
		t.Errorf("golangt %q; want %q", out, "hello")
	}
}
...
func TestUpxCompression(t *testing.T) {
	...
	// Anything below 3.94 is known not to work with golang binaries
	...
	tg.tempFile("main.golang", `package main; import "fmt"; func main() { fmt.Print("hello upx") }`)
	src := tg.path("main.golang")
	obj := tg.path("main")
	...
	if err != nil {
		t.Logf("%s", out)
		t.Fatalf("running compressed golang binary failed with error %s", err)
	}
	if string(out) != "hello upx" {
		t.Fatalf("bad output from compressed golang binary:\ngolangt %q; want %q", out, "hello upx")
	}
}

var golangcacheverify = golangdebug.New("#golangcacheverify")

func TestCacheListStale(t *testing.T) {
	tooSlow(t, "links a binary")
	if golangcacheverify.Value() == "1" {
		t.Skip("GODEBUG golangcacheverify")
	}
	...
	tg.tempFile("golangpath/src/p/p.golang", "package p; import _ \"q\"; func F(){}\n")
	tg.tempFile("golangpath/src/q/q.golang", "package q; func F(){}\n")
	tg.tempFile("golangpath/src/m/m.golang", "package main; import _ \"q\"; func main(){}\n")

	tg.setenv("GOPATH", tg.path("golangpath"))
	tg.run("install", "p", "m")
...
func TestIssue22531(t *testing.T) {
	...
	tg.setenv("GOCACHE", tg.path("cache"))
	tg.tempFile("src/m/main.golang", "package main /* c1 */; func main() {}\n")
	tg.run("install", "-x", "m")
	...
	// be a no-op, thinking the golangfmt binary was up-to-date,
	// even though .Stale could see it was not.
	tg.tempFile("src/m/main.golang", "package main /* c2 */; func main() {}\n")
	tg.run("install", "-x", "m")
	tg.run("list", "-f", "{{.Stale}}", "m")
...
func TestIssue22596(t *testing.T) {
	...
	tg.makeTempdir()
	tg.setenv("GOCACHE", tg.path("cache"))
	tg.tempFile("golangpath1/src/p/p.golang", "package p; func F(){}\n")
	tg.tempFile("golangpath2/src/p/p.golang", "package p; func F(){}\n")

	tg.setenv("GOPATH", tg.path("golangpath1"))
	tg.run("list", "-f={{.Target}}", "p")
	target1 := strings.TrimSpace(tg.getStdout())
	tg.run("install", "p")
	tg.wantNotStale("p", "", "p stale after install")

	tg.setenv("GOPATH", tg.path("golangpath2"))
	tg.run("list", "-f={{.Target}}", "p")
	target2 := strings.TrimSpace(tg.getStdout())
	tg.must(os.MkdirAll(filepath.Dir(target2), 0777))
	tg.must(copyFile(target1, target2, 0666))
	tg.wantStale("p", "build ID mismatch", "p not stale after copy from golangpath1")
	tg.run("install", "p")
...
func TestTestCache(t *testing.T) {
	...
	tg.tempFile("src/p1/p1.golang", "package p1\nvar X =  1\n")
	tg.tempFile("src/p2/p2.golang", "package p2\nimport _ \"p1\"\nvar X = 1\n")
	tg.tempFile("src/t/t1/t1_test.golang", "package t\nimport \"testing\"\nfunc Test1(*testing.T) {}\n")
	tg.tempFile("src/t/t2/t2_test.golang", "package t\nimport _ \"p1\"\nimport \"testing\"\nfunc Test2(*testing.T) {}\n")
	tg.tempFile("src/t/t3/t3_test.golang", "package t\nimport \"p1\"\nimport \"testing\"\nfunc Test3(t *testing.T) {t.Log(p1.X)}\n")
	tg.tempFile("src/t/t4/t4_test.golang", "package t\nimport \"p2\"\nimport \"testing\"\nfunc Test4(t *testing.T) {t.Log(p2.X)}")
	...
	tg.grepStderrNot(`[\\/](compile|gccgolang) `, "incorrectly ran compiler")
	tg.grepStderrNot(`[\\/](link|gccgolang) `, "incorrectly ran linker")
	...
	tg.grepStdout(`ok  \tt/t3\t\(cached\)`, "did not cache t3")
	tg.grepStdout(`ok  \tt/t4\t\(cached\)`, "did not cache t4")
	tg.grepStderrNot(`([\\/](compile|gccgolang) ).*t[0-9]_test\.golang`, "incorrectly ran compiler")
	tg.grepStderrNot(`[\\/](link|gccgolang) `, "incorrectly ran linker")
	...
	tg.grepStderr(`([\\/]compile|gccgolang).*p2.golang`, "did not recompile p2")

	// t1 does not import anything, should not have been rebuilt.
	tg.grepStderrNot(`([\\/]compile|gccgolang).*t1_test.golang`, "incorrectly recompiled t1")
	tg.grepStderrNot(`([\\/]link|gccgolang).*t1_test`, "incorrectly relinked t1_test")
	...
	tg.grepStderr(`([\\/]compile|gccgolang).*t2_test.golang`, "did not recompile t2")
	tg.grepStderr(`([\\/]link|gccgolang).*t2\.test`, "did not relink t2_test")
	// This check does not currently work with gccgolang, as garbage
	...
	tg.grepStderr(`([\\/]compile|gccgolang).*t3_test.golang`, "did not recompile t3")
	tg.grepStderr(`([\\/]link|gccgolang).*t3\.test`, "did not relink t3_test")
	...
	// and not rerun.
	tg.grepStderrNot(`([\\/]compile|gccgolang).*t4_test.golang`, "incorrectly recompiled t4")
	tg.grepStderr(`([\\/]link|gccgolang).*t4\.test`, "did not relink t4_test")
	// This check does not currently work with gccgolang, as garbage
	// collection of unused variables is not turned on by default.
	if runtime.Compiler != "gccgolang" {
...
func TestTestSkipVetAfterFailedBuild(t *testing.T) {
	...
	tg.tempFile("x_test.golang", `package x
		func f() {
			return 1
		}
	`)

	tg.runFail("test", tg.path("x_test.golang"))
...
func TestTestVetRebuild(t *testing.T) {
	...
	// golanglang.org/issue/23701.
	// b_test imports b with augmented method from export_test.golang.
	// b_test also imports a, which imports b.
	// Must not accidentally see un-augmented b propagate through a to b_test.
	tg.tempFile("src/a/a.golang", `package a
		import "b"
		type Type struct{}
		func (*Type) M() b.T {return 0}
	`)
	tg.tempFile("src/b/b.golang", `package b
		type T int
		type I interface {M() T}
	`)
	tg.tempFile("src/b/export_test.golang", `package b
		func (*T) Method() *T { return nil }
	`)
	tg.tempFile("src/b/b_test.golang", `package b_test
		import (
			"testing"
			"a"
			. "b"
...
func TestInstallDeps(t *testing.T) {
	...
	tg.tempFile("src/p1/p1.golang", "package p1\nvar X =  1\n")
	tg.tempFile("src/p2/p2.golang", "package p2\nimport _ \"p1\"\n")
	tg.tempFile("src/main1/main.golang", "package main\nimport _ \"p2\"\nfunc main() {}\n")
...
func main() {
	...
	tg.tempFile("src/a/a_test.golang", `
package main_test
...
func TestV(t *testing.T) {
	...
	tg.tempFile("src/a/p-1.0/p.golang", `
package p
...
func TestBadCommandLines(t *testing.T) {
	...
	tg.tempFile("src/x/x.golang", "package x\n")
	tg.setenv("GOPATH", tg.path("."))

	tg.run("build", "x")

	tg.tempFile("src/x/@y.golang", "package x\n")
	tg.runFail("build", "x")
	tg.grepStderr("invalid input file name \"@y.golang\"", "did not reject @y.golang")
	tg.must(os.Remove(tg.path("src/x/@y.golang")))
	...
	if runtime.Compiler == "gccgolang" {
		tg.runFail("build", "-gccgolangflags=all=@x", "x")
	} else {
	...
	tg.setenv("GOPATH", tg.path("."))
	tg.runFail("build", "@x")
	tg.grepStderr("invalid input directory name \"@x\"|can only use path@version syntax with 'golang get' and 'golang install' in module-aware mode", "did not reject @x directory")

	tg.tempFile("src/@x/y/y.golang", "package y\n")
	tg.setenv("GOPATH", tg.path("."))
	tg.runFail("build", "@x/y")
	tg.grepStderr("invalid import path \"@x/y\"|can only use path@version syntax with 'golang get' and 'golang install' in module-aware mode", "did not reject @x/y import path")
...
func TestTwoPkgConfigs(t *testing.T) {
	...
	defer tg.cleanup()
	tg.parallel()
	tg.tempFile("src/x/a.golang", `package x
		// #cgolang pkg-config: --static a
		import "C"
	`)
	tg.tempFile("src/x/b.golang", `package x
		// #cgolang pkg-config: --static a
		import "C"
	`)
...
echo $* >>`+tg.path("pkg-config.out"))
	...
	if !bytes.Equal(out, []byte(want)) {
		t.Errorf("golangt %q want %q", out, want)
	}
}

func TestCgolangCache(t *testing.T) {
	testenv.MustHaveCGO(t)
	tooSlow(t, "builds a package with cgolang dependencies")
	...
	defer tg.cleanup()
	tg.parallel()
	tg.tempFile("src/x/a.golang", `package main
		// #ifndef VAL
		// #define VAL 0
		// #endif
		// int val = VAL;
...
func TestFilepathUnderCwdFormat(t *testing.T) {
	...
	tg.grepStderrNot(`\.log\.cover\.golang`, "-x output should contain correctly formatted filepath under cwd")
}
...
func TestLinkerTmpDirIsDeleted(t *testing.T) {
	skipIfGccgolang(t, "gccgolang does not use cmd/link")
	testenv.MustHaveCGO(t)
	tooSlow(t, "builds a package with cgolang dependencies")
	...
	defer tg.cleanup()
	tg.parallel()
	tg.tempFile("a.golang", `package main; import "C"; func main() {}`)
	tg.run("build", "-ldflags", "-v", "-o", os.DevNull, tg.path("a.golang"))
	...
	// Find parameter, like "/tmp/golang-link-408556474/golang.o" inside of
	// "host link:" line, and extract temp directory /tmp/golang-link-408556474
	// out of it.
	tmpdir := hostLinkLine
	i := strings.Index(tmpdir, `golang.o"`)
	if i == -1 {
		t.Fatalf(`fail to find "golang.o" in "host link:" line %q`, hostLinkLine)
	}
	tmpdir = tmpdir[:i-1]
...
func TestCoverpkgTestOnly(t *testing.T) {
	skipIfGccgolang(t, "gccgolang has no cover tool")
	tooSlow(t, "links and runs a test binary with coverage enabled")
	...
	tg.tempFile("src/atest/a_test.golang", `
		package a_test
		import ( "a"; "testing" )
...
// Regression test for golanglang.org/issue/34499: version command should not crash
...
func TestExecInDeletedDir(t *testing.T) {
	...
	// `golang version` should not fail
	tg.run("version")
}
