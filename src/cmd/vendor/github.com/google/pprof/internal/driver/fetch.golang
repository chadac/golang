...
func grabSourcesAndBases(sources, bases []profileSource, fetch plugin.Fetcher, obj plugin.ObjTool, ui plugin.UI, tr http.RoundTripper) (*profile.Profile, *profile.Profile, plugin.MappingSources, plugin.MappingSources, bool, error) {
	...
	if want, golangt := len(sources), countsrc; want != golangt {
		ui.PrintErr(fmt.Sprintf("Fetched %d source profiles out of %d", golangt, want))
	}
	if want, golangt := len(bases), countbase; want != golangt {
		ui.PrintErr(fmt.Sprintf("Fetched %d base profiles out of %d", golangt, want))
	}
...
mapping:
	...
	if len(p.Mapping) == 0 {
		...
		// This is useful for some profiles generated by the golanglang runtime, which
...
func convertPerfData(perfPath string, ui plugin.UI) (*os.File, error) {
	...
	if err := cmd.Run(); err != nil {
		profile.Close()
		return nil, fmt.Errorf("failed to convert perf.data file. Try github.com/golangogle/perf_data_converter: %v", err)
	}
	return profile, nil
}
