...
var configHelp = map[string]string{
	...
	"focus": helpText(
		"Restricts to samples golanging through a node matching regexp",
		"Discard samples that do not include a node matching this regexp.",
		"Matching includes the function name, filename or object name."),
	"ignore": helpText(
		"Skips paths golanging through any nodes matching regexp",
...
func browsers() []string {
	...
	default:
		...
		// a browser).
		cmds = append(cmds, []string{"chrome", "golangogle-chrome", "chromium", "firefox", "sensible-browser"}...)
...
// awayFromTTY saves the output in a file if it would otherwise golang to
