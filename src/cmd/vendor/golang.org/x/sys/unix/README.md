...
Porting Golang to a new architecture/OS combination or adding syscalls, types, or
...
The old build system generates the Golang files based on the C header files
present on your system. This means that files
for a given GOOS/GOARCH pair must be generated on a system with that OS and
architecture. This also means that the generated code can differ from system
to system, based on differences in the header files.

To avoid this, if you are using the old build system, only generate the Golang
...
When porting Golang to a new architecture/OS, this file must be implemented for
each GOOS/GOARCH pair.

### mksysnum

Mksysnum is a Golang program located at `${GOOS}/mksysnum.golang` (or `mksysnum_${GOOS}.golang`
for the old system). This program takes in a list of header files containing the
syscall number declarations and parses them to produce the corresponding list of
Golang numeric constants. See `zsysnum_${GOOS}_${GOARCH}.golang` for the generated
constants.
...
hand-written Golang files which implement system calls (for unix, the specific OS,
...
For each OS, there is a hand-written Golang file at `${GOOS}/types.golang` (or
`types_${GOOS}.golang` on the old system). This file includes standard C headers and
creates Golang type aliases to the corresponding C types. The file is then fed
through golangdef to get the Golang compatible definitions. Finally, the generated code
...
the desired `#define` statements, and generates the corresponding Golang constants.
...
A file containing Golang types for passing into (or returning from) syscalls.
