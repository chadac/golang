...
// The duplication of work will golang away once we only use the types2 type checker,
...
func unified(m posMap, noders []*noder) {
	...
	pgolangir.LookupFunc = LookupFunc
	pgolangir.PostLookupCleanup = PostLookupCleanup
...
func readPackage(pr *pkgReader, importpkg *types.Pkg, localStub bool) {
	{
		...
		pkg := r.pkg()
		// This error can happen if "golang tool compile" is called with wrong "-p" flag, see issue #54542.
		if pkg != importpkg {
