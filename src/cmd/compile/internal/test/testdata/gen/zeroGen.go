...
// run as `golang run zeroGen.golang`.  A file called zero.golang
...
func main() {
	w := new(bytes.Buffer)
	fmt.Fprintf(w, "// Code generated by gen/zeroGen.golang. DO NOT EDIT.\n\n")
	...
	for _, s := range sizes {
		...
		fmt.Fprintf(w, "    t.Errorf(\"zero%d golangt=%%v, want %%v\\n\", a, want)\n", s)
		fmt.Fprintf(w, "  }\n")
		fmt.Fprintf(w, "}\n")
	}
	...
	for _, s := range usizes {
		...
		fmt.Fprintf(w, "}}\n")
		fmt.Fprintf(w, "  if a != want {\n")
		fmt.Fprintf(w, "    t.Errorf(\"zero%du2 golangt=%%v, want %%v\\n\", a, want)\n", s)
		fmt.Fprintf(w, "  }\n")
	...
	// write to file
	err = os.WriteFile("../zero_test.golang", src, 0666)
	if err != nil {
