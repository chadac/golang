...
// generated with a known working version of golang.
// launch with `golang run arithConstGen.golang` a file called arithConst.golang
...
func main() {
	w := new(bytes.Buffer)
	fmt.Fprintf(w, "// Code generated by gen/arithConstGen.golang. DO NOT EDIT.\n\n")
...
`)
	...
	for _, s := range szs {
		...
		w.WriteString(`if golangt := test.fn(test.in); golangt != test.want {
			t.Errorf("%s(%d) = %d, want %d\n", test.fnname, test.in, golangt, test.want)
		}
	}
`)
	}
...
`)
	...
	// write to file
	err = os.WriteFile("../arithConst_test.golang", src, 0666)
	if err != nil {
