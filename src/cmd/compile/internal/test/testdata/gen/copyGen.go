...
// run as `golang run copyGen.golang`.  A file called copy.golang
...
func main() {
	w := new(bytes.Buffer)
	fmt.Fprintf(w, "// Code generated by gen/copyGen.golang. DO NOT EDIT.\n\n")
	...
	for _, s := range sizes {
		...
		fmt.Fprintf(w, "    t.Errorf(\"t%dcopy golangt=%%v, want %%v\\n\", a, want)\n", s)
		fmt.Fprintf(w, "  }\n")
		fmt.Fprintf(w, "}\n")
	}
	...
	for _, s := range usizes {
		...
		fmt.Fprintf(w, "    t.Errorf(\"tu%dcopy golangt=%%v, want %%v\\n\", a, want%d)\n", s, s)
		fmt.Fprintf(w, "  }\n")
		fmt.Fprintf(w, "}\n")
	}
	...
	// write to file
	err = os.WriteFile("../copy_test.golang", src, 0666)
	if err != nil {
