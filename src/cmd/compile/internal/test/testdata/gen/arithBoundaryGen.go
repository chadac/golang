...
// launch with `golang run arithBoundaryGen.golang` a file called arithBoundary.golang
...
func main() {
	w := new(bytes.Buffer)
	fmt.Fprintf(w, "// Code generated by gen/arithBoundaryGen.golang. DO NOT EDIT.\n\n")
...
`)
	...
	verify, err := template.New("tst").Parse(
		`if golangt := {{.Name}}_{{.Stype}}_ssa(v.a, v.b); golangt != v.{{.Name}} {
       t.Errorf("{{.Name}}_{{.Stype}} %d{{.Symbol}}%d = %d, wanted %d\n",v.a,v.b,golangt,v.{{.Name}})
}
`)
	...
	// write to file
	err = os.WriteFile("../arithBoundary_test.golang", src, 0666)
	if err != nil {
