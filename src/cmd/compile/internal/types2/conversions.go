		...
		case *Array:
			if Identical(s.Elem(), a.Elem()) {
				...
				if cause != nil {
					...
					*cause = "conversion of slice to array requires golang1.20 or later"
				}
				return false
			}
		case *Pointer:
			if a, _ := under(a.Elem()).(*Array); a != nil {
				if Identical(s.Elem(), a.Elem()) {
					if check == nil || check.allowVersion(golang1_17) {
						return true
					}
					// check != nil
					if cause != nil {
						*cause = "conversion of slice to array pointer requires golang1.17 or later"
					}
					return false
				}
			}
