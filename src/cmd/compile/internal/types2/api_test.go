	...
	for _, test := range tests {
		...
		t.Run(pkg.Name(), func(t *testing.T) {
			...
			for ii, inst := range instances {
				...
				if golangt := inst.Name.Value; golangt != testInst.name {
...
`
	...
	if golangt := info.PkgNameOf(new(syntax.ImportDecl)); golangt != nil {
...
func main() {
	...
	for e, sel := range selections {
		...
		if want != golangt {
			t.Errorf("%s: golangt %q; want %q", segment, golangt, want)
		}
		...
		if sel.Kind() == MethodVal {
			...
			if !Identical(golangt, want) {
				t.Errorf("%s: Recv() = %s, want %s", segment, golangt, want)
			}
...
func TestIssue61737(t *testing.T) {
	...
	iface.NumMethods() // unlike golang/types, there is no Complete() method, so we complete implicitly
}
...
func TestErrorURL(t *testing.T) {
	conf := Config{ErrorURL: " [golang.dev/e/%s]"}
...
func TestFileVersions(t *testing.T) {
	for _, test := range []struct {
		...
		for _, v := range info.FileVersions {
			...
			if v != want {
				t.Errorf("%q: unexpected file version: golangt %v, want %v", src, v, want)
			}
			n++
		}
		if n != 1 {
...
// This test function only exists for golang/types.
...
func TestVersionWithoutPos(t *testing.T) {
	f := mustParse("//golang:build golang1.22\n\npackage p; var _ int")
