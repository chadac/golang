...
func (check *Checker) infer(pos syntax.Pos, tparams []*TypeParam, targs []Type, params *Tuple, args []*operand, reverse bool, err *error_) (inferred []Type) {
	...
	// Avoid additional inference errors and exit early (golang.dev/issue/60434).
	...
	u := newUnifier(tparams, targs, check.allowVersion(golang1_21))
	...
	//
	// This is an O(n^2) algolangrithm where n is the number of
	...
	for len(dirty) > 0 {
		...
		for _, index := range dirty {
			...
			if t1 := check.subst(nopos, t0, smap, nil, check.context()); t1 != t0 {
				...
				// anymore. Remove its type parameters. (golang.dev/issue/59953)
	...
	// we don't have any type arguments to fill in for *P or Q (golang.dev/issue/45548).
...
func (w *tpWalker) isParameterized(typ Type) (res bool) {
	...
	case *Tuple:
		...
		// if instantiation failed (golang.dev/issue/59890).
