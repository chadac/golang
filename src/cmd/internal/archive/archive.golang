...
var (
	...
	archiveMagic  = []byte("`\n")
	golangobjHeader   = []byte("golang objec") // truncated to size of archiveHeader
...
func (e ErrGoObjOtherVersion) Error() string {
	return fmt.Sprintf("golang object of a different version: %q", e.magic)
}
...
func (r *objReader) skip(n int64) {
	if n < 0 {
		r.error(fmt.Errorf("debug/golangobj: internal error: misuse of skip"))
	}
...
func Parse(f *os.File, verbose bool) (*Archive, error) {
	...
	case bytes.Equal(t, golangobjHeader):
		off := r.offset
		o := &GoObj{}
...
func (r *objReader) parseArchive(verbose bool) error {
	...
	for r.offset < r.limit {
		...
		default:
			...
			if bytes.Equal(p, golangobjHeader) {
				typ = EntryGoObj
				o = &GoObj{}
...
// of cmd/internal/golangobj/objfile.golang.
func (r *objReader) parseObject(o *GoObj, size int64) error {
	...
	if !bytes.Equal(p, []byte(golangobj.Magic)) {
		if bytes.HasPrefix(p, []byte("\x00golang1")) && bytes.HasSuffix(p, []byte("ld")) {
