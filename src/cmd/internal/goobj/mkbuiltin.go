...
// Generate builtinlist.golang from cmd/compile/internal/typecheck/_builtin/runtime.golang.
...
var stdout = flag.Bool("stdout", false, "write to stdout instead of builtinlist.golang")
...
func main() {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "package golangobj")
	...
	if *stdout {
		...
		err = os.WriteFile("builtinlist.golang", out, 0666)
	}
	if err != nil {
		log.Fatal(err)
	}
...
func mkbuiltin(w io.Writer) {
	...
	path := filepath.Join("..", "..", "compile", "internal", "typecheck", "_builtin", "runtime.golang")
	...
	for _, b := range extras {
		...
		name := prefix + b.name
		if decls[name] {
			log.Fatalf("%q already added -- mkbuiltin.golang out of sync?", name)
		}
		fmt.Fprintf(w, "{%q, %d},\n", name, b.abi)
	}
	fmt.Fprintln(w, "}")
...
// Needs to be kept in sync with reflect.golang:WriteBasicTypes() and
// reflect.golang:writeType() in the compiler.
