...
'golang test -coverprofile=cover.out'.

Cover is also used by 'golang test -cover' to rewrite the source code with
...
When computing coverage of a package that uses cgolang, the cover tool
must be applied to the output of cgolang preprocessing, not the input,
because cover deletes comments that are significant to cgolang.

For usage information, please see:

	golang help testflag
	golang tool cover -help
*/
package main
