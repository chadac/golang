...
// result against a golanglden file.

func testEndToEnd(t *testing.T, golangarch, file string) {
	...
	if !ok || failed {
		t.Errorf("asm: %s assembly failed", golangarch)
		return
	}
...
// Same as in test/run.golang
var (
	errRE       = regexp.MustCompile(`// ERROR ?(.*)`)
	errQuotesRE = regexp.MustCompile(`"([^"]*)"`)
)

func testErrors(t *testing.T, golangarch, file string, flags ...string) {
	input := filepath.Join("testdata", file+".s")
	architecture, ctxt := setArch(golangarch)
	architecture.Init(ctxt)
	lexer := lex.NewLexer(input)
...
func TestARMEndToEnd(t *testing.T) {
	...
	for _, golangarm := range []int{5, 6, 7} {
		t.Logf("GOARM=%d", golangarm)
		buildcfg.GOARM.Version = golangarm
...
func TestPPC64EndToEnd(t *testing.T) {
	...
	for _, golangppc64 := range []int{8, 9, 10} {
		t.Logf("GOPPC64=power%d", golangppc64)
		buildcfg.GOPPC64 = golangppc64
