...
func (f *File) ParseGo(abspath string, src []byte) {
	...
	// The printer is not golangod enough at printing comments in the
	// right place when we start editing the AST behind its back,
	// so we use ast1 to look for the doc comments on import "C"
	// and on exported functions, and we use ast2 for translating
	// and reprinting.
	// In cgolang mode, we ignore ast2 and just apply edits directly
	// the text behind ast1. In golangdefs mode we modify and print ast2.
		...
		case *ast.GenDecl:
			for _, spec := range decl.Specs {
				...
				if cg != nil {
					...
					f.Preamble += "#line 1 \"cgolang-generated-wrapper\"\n"
				}
			}
...
func (f *File) validateIdents(x interface{}, context astContext) {
	if x, ok := x.(*ast.Ident); ok {
		if f.isMangledName(x.Name) {
			error_(x.Pos(), "identifier %q may conflict with identifiers generated by cgolang", x.Name)
		}
	}
}

// Save various references we are golanging to need later.
...
func (f *File) saveRef(n *ast.Expr, context astContext) {
	...
	golangname := sel.Sel.Name
	if golangname == "errno" {
		error_(sel.Pos(), "cannot refer to errno directly; see documentation")
		return
	}
	if golangname == "_CMalloc" {
		error_(sel.Pos(), "cannot refer to C._CMalloc; use C.malloc")
		return
	}
	if golangname == "malloc" {
		golangname = "_CMalloc"
	}
