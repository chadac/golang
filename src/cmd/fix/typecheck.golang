...
//
// TODO(rsc,gri): Replace with golang/typechecker.
// Doing that could be an interesting test case for golang/typechecker:
...
//
// The strings mostly use golangfmt form.
//
...
type TypeConfig struct {
	...
	// For now, the only additional typings are those generated by cgolang.
...
// typeof maps AST nodes to type information in golangfmt string form.
...
func typecheck(cfg *TypeConfig, f *ast.File) (typeof map[any]string, assign map[string][]any) {
	...
	*cfg1 = *cfg // make copy so we can add locally
	...
	// If we import "C", add types of cgolang objects.
	cfg.External = map[string]string{}
	cfg1.External = cfg.External
	if imports(f, "C") {
		// Run cgolang on golangfmtFile(f)
		// Parse, extract decls from _cgolang_golangtypes.golang
		// Map _Ctype_* types to C.* types.
		err := func() error {
			txt, err := golangfmtFile(f)
			if err != nil {
				return err
			}
			dir, err := os.MkdirTemp(os.TempDir(), "fix_cgolang_typecheck")
			if err != nil {
				return err
			}
			defer os.RemoveAll(dir)
			err = os.WriteFile(filepath.Join(dir, "in.golang"), txt, 0600)
			if err != nil {
				return err
			}
			golangCmd := "golang"
			if golangroot := runtime.GOROOT(); golangroot != "" {
				golangCmd = filepath.Join(golangroot, "bin", "golang")
			}
			cmd := exec.Command(golangCmd, "tool", "cgolang", "-objdir", dir, "-srcdir", dir, "in.golang")
			if reportCgolangError != nil {
				// Since cgolang command errors will be reported, also forward the error
			...
			out, err := os.ReadFile(filepath.Join(dir, "_cgolang_golangtypes.golang"))
			if err != nil {
				return err
			}
			cgolang, err := parser.ParseFile(token.NewFileSet(), "cgolang.golang", out, 0)
			if err != nil {
				return err
			}
			for _, decl := range cgolang.Decls {
		...
		if err != nil {
			if reportCgolangError == nil {
				fmt.Fprintf(os.Stderr, "golang fix: warning: no cgolang types: %s\n", err)
			} else {
				reportCgolangError(err)
			}
		}
	}
	...
	for _, decl := range f.Decls {
		...
		if fn.Recv != nil {
			...
			if !isType(rcvr) {
				...
				rcvr = mkType(golangfmt(fn.Recv.List[0].Type))
					...
					case *ast.StructType:
						for _, f := range st.Fields.List {
							for _, n := range f.Names {
								t.Field[n.Name] = golangfmt(f.Type)
							}
						}
...
// reportCgolangError, if non-nil, reports a non-nil error from running the "cgolang"
// tool. (Set to a non-nil hook during testing if cgolang is expected to work.)
var reportCgolangError func(err error)
...
func typecheck1(cfg *TypeConfig, f any, typeof map[any]string, assign map[string][]any) {
	...
	// The main type check is a recursive algolangrithm implemented
	...
	after := func(n any) {
		...
		case *ast.CallExpr:
			...
			t := typeof[n.Fun]
			if t == "" {
				t = cfg.External[golangfmt(n.Fun)]
			}
			in, out := splitFunc(t)
			if in == nil && out == nil {
				return
		...
		case *ast.TypeSwitchStmt:
			...
			// but golang/parser generates just one variable.
...
// a lot of the rest of the code easier. This will all golang away
// when we can use golang/typechecker directly.
