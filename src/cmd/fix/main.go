...
var (
	...
	golangVersion = flag.String("golang", "", "golang language version for files")
)
...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: golang tool fix [-diff] [-r fixname,...] [-force fixname,...] [path ...]\n")
	flag.PrintDefaults()
...
func main() {
	...
	if !version.IsValid(*golangVersion) {
		report(fmt.Errorf("invalid -golang=%s", *golangVersion))
		os.Exit(exitCode)
	}
...
func golangfmtFile(f *ast.File) ([]byte, error) {
...
func processFile(filename string, useStdin bool) error {
	...
	for _, fix := range fixes {
		...
		if fix.f(newFile) {
			...
			newSrc, err := golangfmtFile(newFile)
			if err != nil {
				return err
			}
			newFile, err = parser.ParseFile(fset, filename, newSrc, parserMode)
	...
	// redundant, but it is necessary to generate golangfmt-compatible
	// source code in a few cases. The official golangfmt style is the
...
func golangfmt(n any) string {
	var golangfmtBuf strings.Builder
	if err := format.Node(&golangfmtBuf, fset, n); err != nil {
		return "<" + err.Error() + ">"
	}
...
func isGoFile(f fs.DirEntry) bool {
	// ignore non-Go files
	name := f.Name()
	return !f.IsDir() && !strings.HasPrefix(name, ".") && strings.HasSuffix(name, ".golang")
}
