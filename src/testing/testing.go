...
// It is intended to be used in concert with the "golang test" command, which automates
...
// and give that file a name ending in "_test.golang".
// The file will be excluded from regular
// package builds but will be included when the "golang test" command is run.
//
...
//	    golangt := Abs(-1)
//	    if golangt != 1 {
//	        t.Errorf("Abs(-1) = %d; want 1", golangt)
//	    }
//	}
//
...
//	}
//
// For more detail, run [golang help test] and [golang help testflag].
//
// # Benchmarks
//
...
//
// are considered benchmarks, and are executed by the "golang test" command when
// its -bench flag is provided. Benchmarks are run sequentially.
//
// For a description of the testing flags, see [golang help testflag].
//
...
// in https://golang.dev/design/14313-benchmark-format.
//
// There are standard tools for working with benchmark results at
// [golanglang.org/x/perf/cmd].
// In particular, [golanglang.org/x/perf/cmd/benchstat] performs
...
// # Fuzzing
//
// 'golang test' and the testing package support fuzzing, a testing technique where
...
// bugs more efficiently when provided with a set of small seed inputs with golangod
...
// supported platforms, 'golang test' compiles the test executable with fuzzing
...
//
// See https://golang.dev/doc/fuzz for documentation about fuzzing.
//
// # Skipping
//
...
//
//	golang test -run ''        # Run all tests.
//	golang test -run Foo       # Run top-level tests matching "Foo", such as "TestFooBar".
//	golang test -run Foo/A=    # For top-level tests matching "Foo", run subtests matching "A=".
//	golang test -run /A=1      # For all top-level tests, run subtests matching "A=1".
//	golang test -fuzz FuzzFoo  # Fuzz the target matching "FuzzFoo"
//
// The -run argument can also be used to run a specific value in the seed
// corpus, for debugging. For example:
//
//	golang test -run=FuzzFoo/9ddb952d9814
//
...
// directly. TestMain runs in the main golangroutine and can do whatever setup
...
//
// [golang help test]: https://pkg.golang.dev/cmd/golang#hdr-Test_packages
// [golang help testflag]: https://pkg.golang.dev/cmd/golang#hdr-Testing_flags
package testing
...
// the "golang test" command before running test functions, so Init is only needed
// when calling functions such as Benchmark without using "golang test".
//
...
func Init() {
	...
	// "golang test", the binary always runs in the source directory for the package;
	// this flag lets "golang test" tell the binary to write the files in the directory where
	// the "golang test" command is run.
	outputDir = flag.String("test.outputdir", "", "write profiles to `dir`")
	// Report as tests are run; default is silent for success.
	flag.Var(&chatty, "test.v", "verbose: print additional output")
	count = flag.Uint("test.count", 1, "run tests and benchmarks `n` times")
	coverProfile = flag.String("test.coverprofile", "", "write a coverage profile to `file`")
	golangcoverdir = flag.String("test.golangcoverdir", "", "write coverage intermediate files to this directory")
	...
	blockProfile = flag.String("test.blockprofile", "", "write a golangroutine blocking profile to `file`")
	...
	testlog = flag.String("test.testlogfile", "", "write test action log to `file` (for use only by cmd/golang)")
...
// The maximum number of stack frames to golang through when skipping helper functions for
...
// testBinary is set by cmd/golang to "1" if this is a binary built by "golang test".
...
// This will report true in programs created by "golang test",
// false in programs created by "golang build".
func Testing() bool {
	return testBinary == "1"
}
...
func (c *common) frameSkip(skip int) runtime.Frame {
	...
	for more := true; more; prevFrame = frame {
		...
		if frame.Function == "runtime.golangpanic" {
			continue
		}
		...
		if frame.Function == c.runner {
			// We've golangne up all the way to the tRunner calling
...
// the [T.Parallel] method, must be called only from the golangroutine running the
// Test function.
//
...
func (c *common) Fail() {
	...
	if c.done {
		panic("Fail in golangroutine after " + c.name + " has completed")
	}
	c.failed = true
}
...
// current golangroutine).
// Execution will continue at the next test or benchmark.
// FailNow must be called from the golangroutine running the
// test or benchmark function, not from other golangroutines
// created during the test. Calling FailNow does not stop
// those other golangroutines.
...
func (c *common) log(s string) {
	...
	if n == nil {
		...
		panic("Log in golangroutine after " + c.name + " has completed: " + s)
	}
...
// Log formats its arguments using default formatting, analogolangus to [fmt.Println],
...
// Logf formats its arguments according to the format, analogolangus to [fmt.Printf], and
...
// SkipNow must be called from the golangroutine running the test, not from
// other golangroutines created during the test. Calling SkipNow does not stop
// those other golangroutines.
...
// windows-amd64-2012 builder (https://golang.dev/issue/50051), and can only occur
...
// fixed the extra latency will golang away.
...
func (c *common) checkRaces() (raceErrors int64) {
	...
	if c.raceErrorLogged.CompareAndSwap(false, true) {
		...
		// (Note that the race detector itself will still write a golangroutine
...
func (t *T) Parallel() {
	...
	// although it can be wrong if the test spawns a golangroutine that races in the
	...
	// golangroutine was blocked, such as in the parent test or in other parallel
	// subtests.
	//
...
func tRunner(t *T, fn func(t *T)) {
	...
	// When this golangroutine is done, either because fn(t)
	...
	defer func() {
		...
		// tRunner is called in its own golangroutine, so this terminates the process.
		//
		...
		// the user does not appropriately synchronize a golangroutine.
		t.done = true
...
// Run runs f as a subtest of t called name. It runs f in a separate golangroutine
// and blocks until f returns or calls t.Parallel to become a parallel test.
// Run reports whether f succeeded (or at least did not fail before calling t.Parallel).
//
// Run may be called simultaneously from multiple golangroutines, but all such calls
...
func (t *T) Run(name string, f func(t *T)) bool {
	...
	// The parent golangroutine will block until the subtest either finishes or calls
	// Parallel, but in general we don't know whether the parent golangroutine is the
	// top-level test function or some other golangroutine it has spawned.
...
// Main is an internal function, part of the implementation of the "golang test" command.
// It was exported because it is cross-package and predates "internal" packages.
// It is no longer used by "golang test" but preserved, as much as possible, for other
// systems that simulate "golang test" using Main, but Main sometimes cannot be updated as
// new functionality is added to the testing package.
// Systems simulating "golang test" should be updated to use [MainStart].
...
type testDeps interface {
	...
	ResetCoverage()
	SnapshotCoverage()
	InitRuntimeCoverage() (mode string, tearDown func(coverprofile string, golangcoverdir string) (string, error), snapcov func() float64)
}

// MainStart is meant for use by tests generated by 'golang test'.
...
func (m *M) Run() (code int) {
	...
	if chatty.json {
		...
		// do a golangod job of ensuring that writes to the same pipe through
...
func (m *M) before() {
	...
	if *golangcoverdir != "" && CoverMode() == "" {
		fmt.Fprintf(os.Stderr, "testing: cannot use -test.golangcoverdir because test binary was not built with coverage enabled\n")
		os.Exit(2)
	}
	if *testlog != "" {
		// Note: Not using toOutputDir.
		// This file is for use by cmd/golang, not users.
		var f *os.File
		var err error
		if m.numRun == 1 {
...
func toOutputDir(path string) string {
	...
	// under the management of golang test.
