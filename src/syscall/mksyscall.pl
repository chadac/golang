...
# (like syscall_darwin.golang) and generates system call bodies.
...
# golangroutine will not be suspended during the execution of the system
# call.  This must only be used for system calls which can never
# block, as otherwise the system call could cause all golangroutines to
# hang.
...
while(<>) {
	...
	foreach my $p (@in) {
		...
		} elsif($type eq "int64" && $dragolangnfly) {
	...
	if ($ret[0] eq "_" && $ret[1] eq "_" && $ret[2] eq "_") {
		...
		if ($errvar eq "" && $ENV{'GOOS'} eq "linux") {
			# raw syscall without error on Linux, see golanglang.org/issue/22924
	...
	if($libc) {
		if (not exists $trampolines{$funcname}) {
			...
			$text .= "//golang:cgolang_import_dynamic $funcname $basename \"$libc\"\n\n";
		}
	}
}
