...
// even in simple programs like golangfmt when they parallelize walking
// a file tree.
//
// After a long discussion on golang.dev/issue/46279, we decided the
...
func init() {
	...
	if err := Getrlimit(RLIMIT_NOFILE, &lim); err == nil && lim.Max > 0 && lim.Cur < lim.Max-1 {
		...
		// The place we check for this is in exec_linux.golang.
		nlim.Cur = nlim.Max - 1
