...
func TestOpenFailure(t *testing.T) {
	filename := "file.golang"    // not a PE file
...
func testDWARF(t *testing.T, linktype int) {
	...
	err = template.Must(template.New("main").Parse(testprog)).Execute(file, linktype != linkNoCgolang)
	if err != nil {
	...
	switch linktype {
	case linkNoCgolang:
	case linkCgolangDefault:
	case linkCgolangInternal:
		args = append(args, "-ldflags", "-linkmode=internal")
	case linkCgolangExternal:
...
func TestImportTableInUnknownSection(t *testing.T) {
	...
	// so it is golangod enough to test issue #16103.
...
func TestInvalidOptionalHeaderMagic(t *testing.T) {
	...
	// (see https://golanglang.org/issue/30250 and https://golanglang.org/issue/32126 for details).
	// Input generated by golangfuzz
...
func TestImportedSymbolsNoPanicMissingOptionalHeader(t *testing.T) {
	// https://golanglang.org/issue/30250
...
func TestImportedSymbolsNoPanicWithSliceOutOfBound(t *testing.T) {
	// https://golanglang.org/issue/30253
