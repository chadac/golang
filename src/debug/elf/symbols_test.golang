	...
	do := func(file string, ts []Symbol, getfunc func(*File) ([]Symbol, error)) {
		...
		for i, s := range fs {
			if s.HasVersion {
				...
				if golangt, want := s.VersionIndex.Index(), uint16(s.VersionIndex); golangt != want {
					t.Errorf("%s: symbol %d: VersionIndex.Index() == %d, want %d", file, i, golangt, want)
				}
			}
		}
...
// golanglden symbol table data generated by testdata/getgolangldsym.c
...
var symbolsGolden = map[string][]Symbol{
	...
	"testdata/golang-relocation-test-clang-x86.obj": {
		Symbol{
			Name:         "golang-relocation-test-clang.c",
			Info:         0x4,
			Other:        0x0,
			HasVersion:   false,
			VersionIndex: 0,
