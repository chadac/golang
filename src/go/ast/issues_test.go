...
func TestIssue33649(t *testing.T) {
	for _, src := range []string{
		...
		if fdEnd != tfEnd {
			t.Errorf("%q: golangt fdEnd = %d; want %d (base = %d, size = %d)", src, fdEnd, tfEnd, tf.Base(), tf.Size())
		}
	}
}
...
// Package p doc comment golanges here.
//
// Code generated by gen. DO NOT EDIT.
package p
...
`, true},
		...
		golangt := ast.IsGenerated(f)
		if golangt != test.want {
			t.Errorf("%d: IsGenerated on <<%s>> returned %t", i, test.src, golangt)
		}
	}
