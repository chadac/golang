...
// Comment directives like "//line" and "//golang:noinline" are also removed.
...
func (g *CommentGroup) Text() string {
	...
	for _, c := range comments {
		...
		case '/':
			...
			if isDirective(c) {
				// Ignore //golang:noinline, //line, and so on.
				continue
			}
		case '*':
			/*-style comment */
...
type (
	...
	FuncType struct {
		...
		Results    *FieldList // (outgolanging) results; or nil
	}
...
type (
	...
	// A GoStmt node represents a golang statement.
	GoStmt struct {
		Go   token.Pos // position of "golang" keyword
		Call *CallExpr
	}
	...
	// A BranchStmt node represents a break, continue, golangto,
...
// golang/format and golang/printer), special care must be taken to update comments
...
//
// [#18593]: https://golang.dev/issue/18593
// [#20744]: https://golang.dev/issue/20744
type File struct {
	Doc     *CommentGroup // associated documentation; or nil
	...
	GoVersion          string          // minimum Go version required by //golang:build or // +build directives
}
...
// at https://golang.dev/s/generatedcode.
//
