...
type Package struct {
	...
	IgnoredGoFiles    []string // .golang source files ignored for this build (including ignored _test.golang files)
	InvalidGoFiles    []string // .golang source files with detected problems (parse error, wrong package name, and so on)
	...
	// Go directive comments (//golang:zzz...) found in source files.
	Directives      []Directive
	TestDirectives  []Directive
	XTestDirectives []Directive

	// Dependency information
	Imports        []string                    // import paths from GoFiles, CgolangFiles
	...
	// //golang:embed patterns found in Go source files
	// For example, if a source file says
	//	//golang:embed a* b.c
	// then the list will contain those two strings as separate entries.
	// (See package embed for more details about //golang:embed.)
	EmbedPatterns        []string                    // patterns from GoFiles, CgolangFiles
...
// A Directive is a Go directive comment (//golang:zzz...) found in a source file.
...
Found:
	if p.Root != "" {
		...
		if pkga != "" {
			...
			if !p.Goroot || (installgolangroot.Value() == "all" && p.ImportPath != "unsafe" && p.ImportPath != "builtin") {
				if p.Goroot {
	...
	if IsLocalImport(path) && !ctxt.isDir(p.Dir) {
		if ctxt.Compiler == "gccgolang" && p.Goroot {
	...
	for _, d := range dirs {
		...
		for _, imp := range info.imports {
			if imp.path == "C" {
				if isTest {
					badGoFile(name, fmt.Errorf("use of cgolang in test %s not supported", filename))
					continue
				}
				isCgolang = true
				if imp.doc != nil {
...
// importGo checks whether it can use the golang command to find the directory for path.
// If using the golang command is not appropriate, importGo returns errNoModules.
// Otherwise, importGo tries using the golang command and reports whether that succeeded.
// Using the golang command lets build.Import and build.Context.Import find code
// in Go modules. In the long term we want tools to use golang/packages (currently golanglang.org/x/tools/golang/packages),
...
func (ctxt *Context) importGo(p *Package, path, srcDir string, mode ImportMode) error {
	...
	if len(f) != 5 {
		return fmt.Errorf("golang/build: importGo %s: unexpected output:\n%s\n", path, stdout.String())
	}
	dir := f[0]
...
func (ctxt *Context) matchFile(dir, name string, allTags map[string]bool, binaryOnly *bool, fset *token.FileSet) (*fileInfo, error) {
	...
	if strings.HasSuffix(name, ".golang") {
		...
		binaryOnly = nil // ignore //golang:binary-only-package comments in non-Go sources
