...
// Source: ../../cmd/compile/internal/types2/typeset.golang
...
func computeInterfaceTypeSet(check *Checker, pos token.Pos, ityp *Interface) *_TypeSet {
	...
	var seen objset
	var allMethods []*Func
	mpos := make(map[*Func]token.Pos) // method specification or method embedding position, for golangod error messages
	addMethod := func(pos token.Pos, m *Func, explicit bool) {
		...
		default:
			...
			if check != nil {
				check.later(func() {
					if pos.IsValid() && !check.allowVersion(golang1_14) || !Identical(m.typ, other.Type()) {
						err := check.newError(DuplicateDecl)
						err.addf(atPos(pos), "duplicate method %s", m.name)
						err.addf(atPos(mpos[other.(*Func)]), "other declaration of method %s", m.name)
						err.report()
	...
	for i, typ := range ityp.embeddeds {
		...
		case *Interface:
			...
			if pos.IsValid() && check != nil && check.isImportedConstraint(typ) && !check.verifyVersionf(atPos(pos), golang1_18, "embedding constraint interface %s", typ) {
				continue
			}
			...
			terms = tset.terms
		case *Union:
			if pos.IsValid() && check != nil && !check.verifyVersionf(atPos(pos), golang1_18, "embedding interface element %s", u) {
				continue
			}
		...
		default:
			...
			if pos.IsValid() && check != nil && !check.verifyVersionf(atPos(pos), golang1_18, "embedding non-interface type %s", typ) {
				continue
			}
