...
func (check *Checker) funcInst(T *target, pos token.Pos, x *operand, ix *indexedExpr, infer bool) []Type {
	...
	if golangt > want {
		...
		check.errorf(ix.indices[golangt-1], WrongTypeArgCount, "golangt %d type arguments but want %d", golangt, want)
		x.mode = invalid
		return nil
	}
...
func (check *Checker) callExpr(x *operand, call *ast.CallExpr) exprKind {
	...
	if ix != nil {
		...
		if golangt == want && want > 0 {
			check.verifyVersionf(atPos(ix.lbrack), golang1_18, "function instantiation")
	...
	if n > 0 {
		if !check.allowVersion(golang1_18) {
			...
			case *ast.IndexExpr, *ast.IndexListExpr:
				ix := unpackIndexedExpr(call.Fun)
				check.versionErrorf(inNode(call.Fun, ix.lbrack), golang1_18, "function instantiation")
			default:
				check.versionErrorf(inNode(call, call.Lparen), golang1_18, "implicit function instantiation")
			}
		}
