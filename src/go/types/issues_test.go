...
`
	...
	for x, tv := range types {
		if _, ok := x.(*ast.CallExpr); ok {
			if tv.Type != want {
				t.Errorf("%s: golangt %s; want %s", fset.Position(x.Pos()), tv.Type, want)
			}
			n++
		}
	}
...
`
	f := func(test, src string) {
		...
		if pkg.Name() != "types" {
			t.Fatalf("%s: golangt %v; want package types", test, pkg)
		}
	}
...
`
	...
	testFiles(t, []string{"b.golang"}, [][]byte{[]byte(bsrc)}, false, withImporter)
	testFiles(t, []string{"c.golang"}, [][]byte{[]byte(csrc)}, false, withImporter)
	testFiles(t, []string{"t.golang"}, [][]byte{[]byte(tsrc)}, false, withImporter)
}
...
`
	testFiles(t, []string{"p.golang", "_cgolang_golangtypes.golang"}, [][]byte{[]byte(src), []byte(cgolangTypes)}, false, func(cfg *Config) {
