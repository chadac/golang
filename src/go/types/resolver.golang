...
func (check *Checker) importPackage(at positioner, path, dir string) *Package {
	...
	if path == "C" && (check.conf.FakeImportC || check.conf.golang115UsesCgolang) {
		if check.conf.FakeImportC && check.conf.golang115UsesCgolang {
			check.error(at, BadImportPath, "cannot use FakeImportC and golang115UsesCgolang together")
		}
		imp = NewPackage("C", "C")
	...
	for fileNo, file := range check.files {
		...
		check.walkDecls(file.Decls, func(d decl) {
			...
			case funcDecl:
				...
				_ = d.decl.Type.TypeParams.NumFields() != 0 && !hasTParamError && check.verifyVersionf(d.decl.Type.TypeParams.List[0], golang1_18, "type parameter")
...
func (check *Checker) resolveBaseTypeName(ptr bool, name *ast.Ident) (ptr_ bool, base *TypeName) {
	...
	for name != nil {
		...
		// The golang/parser keeps parentheses; strip them, if any.
