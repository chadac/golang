...
// This file implements a custom generator to create various golang/types
...
var filesToWrite = flag.String("write", "", `golang/types files to generate, or "all" for all files`)

const (
	srcDir = "/src/cmd/compile/internal/types2/"
	dstDir = "/src/golang/types/"
)

// TestGenerate verifies that generated files in golang/types match their types2
// counterpart. If -write is set, this test actually writes the expected
// content to golang/types; otherwise, it just compares with the existing content.
...
func generate(t *testing.T, filename string, write bool) {
	...
	fmt.Fprintf(&buf, "// Code generated by \"golang test -run=Generate -write=all\"; DO NOT EDIT.\n")
...
var filemap = map[string]action{
	"alias.golang": fixTokenPos,
	"assignments.golang": func(f *ast.File) {
		renameImportPath(f, `"cmd/compile/internal/syntax"->"golang/ast"`)
	...
	"array.golang":          nil,
	"api_predicates.golang": nil,
	"basic.golang":          nil,
	"builtins.golang": func(f *ast.File) {
		renameImportPath(f, `"cmd/compile/internal/syntax"->"golang/ast"`)
	...
	"builtins_test.golang": func(f *ast.File) {
		renameImportPath(f, `"cmd/compile/internal/syntax"->"golang/ast"`, `"cmd/compile/internal/types2"->"golang/types"`)
		renameSelectorExprs(f, "syntax.Name->ast.Ident", "p.Value->p.Name") // must happen before renaming identifiers
		renameIdents(f, "syntax->ast")
	},
	"chan.golang":         nil,
	"const.golang":        fixTokenPos,
	"context.golang":      nil,
	"context_test.golang": nil,
	"conversions.golang":  nil,
	"errors_test.golang":  func(f *ast.File) { renameIdents(f, "nopos->noposn") },
	"errsupport.golang":   nil,
	"gccgolangsizes.golang":   nil,
	"gcsizes.golang":      func(f *ast.File) { renameIdents(f, "IsSyncAtomicAlign64->_IsSyncAtomicAlign64") },
	"hilbert_test.golang": func(f *ast.File) { renameImportPath(f, `"cmd/compile/internal/types2"->"golang/types"`) },
	"infer.golang":        func(f *ast.File) { fixTokenPos(f); fixInferSig(f) },
	"initorder.golang":    nil,
	// "initorder.golang": fixErrErrorfCall, // disabled for now due to unresolved error_ use implications for golangpls
	"instantiate.golang":      func(f *ast.File) { fixTokenPos(f); fixCheckErrorfCall(f) },
	"instantiate_test.golang": func(f *ast.File) { renameImportPath(f, `"cmd/compile/internal/types2"->"golang/types"`) },
	"literals.golang": func(f *ast.File) {
		insertImportPath(f, `"golang/token"`)
		renameImportPath(f, `"cmd/compile/internal/syntax"->"golang/ast"`)
	...
	"lookup.golang":    func(f *ast.File) { fixTokenPos(f) },
	"main_test.golang": nil,
	"map.golang":       nil,
	"mono.golang": func(f *ast.File) {
		fixTokenPos(f)
		insertImportPath(f, `"golang/ast"`)
		renameSelectorExprs(f, "syntax.Expr->ast.Expr")
	},
	"named.golang":  func(f *ast.File) { fixTokenPos(f); renameSelectors(f, "Trace->_Trace") },
	"object.golang": func(f *ast.File) { fixTokenPos(f); renameIdents(f, "NewTypeNameLazy->_NewTypeNameLazy") },
	// TODO(gri) needs adjustments for TestObjectString - disabled for now
	// "object_test.golang": func(f *ast.File) { renameImportPath(f, `"cmd/compile/internal/types2"->"golang/types"`) },
	"objset.golang": nil,
	"operand.golang": func(f *ast.File) {
		insertImportPath(f, `"golang/token"`)
		renameImportPath(f, `"cmd/compile/internal/syntax"->"golang/ast"`)
		renameSelectorExprs(f,
	...
	"package.golang":    nil,
	"pointer.golang":    nil,
	"predicates.golang": nil,
	"range.golang": func(f *ast.File) {
		renameImportPath(f, `"cmd/compile/internal/syntax"->"golang/ast"`)
	...
	"recording.golang": func(f *ast.File) {
		renameImportPath(f, `"cmd/compile/internal/syntax"->"golang/ast"`)
		renameSelectorExprs(f, "syntax.Name->ast.Ident") // must happen before renaming identifiers
		renameIdents(f, "syntax->ast")
		fixAtPosCall(f)
	},
	"scope.golang":         func(f *ast.File) { fixTokenPos(f); renameIdents(f, "InsertLazy->_InsertLazy") },
	"selection.golang":     nil,
	"sizes.golang":         func(f *ast.File) { renameIdents(f, "IsSyncAtomicAlign64->_IsSyncAtomicAlign64") },
	"slice.golang":         nil,
	"subst.golang":         func(f *ast.File) { fixTokenPos(f); renameSelectors(f, "Trace->_Trace") },
	"termlist.golang":      nil,
	"termlist_test.golang": nil,
	"tuple.golang":         nil,
	"typelists.golang":     nil,
	"typeset.golang":       func(f *ast.File) { fixTokenPos(f); renameSelectors(f, "Trace->_Trace") },
	"typeparam.golang":     nil,
	"typeterm_test.golang": nil,
	"typeterm.golang":      nil,
	"typestring.golang":    nil,
	"under.golang":         nil,
	"unify.golang":         fixSprintf,
	"universe.golang":      fixGlobalTypVarDecl,
	"util_test.golang":     fixTokenPos,
	"validtype.golang":     func(f *ast.File) { fixTokenPos(f); renameSelectors(f, "Trace->_Trace") },
}

// TODO(gri) We should be able to make these rewriters more configurable/composable.
//           For now this is a golangod starting point.
...
// fixTokenPos changes imports of "cmd/compile/internal/syntax" to "golang/token",
// uses of syntax.Pos to token.Pos, and calls to x.IsKnown() to x.IsValid().
func fixTokenPos(f *ast.File) {
	m := makeRenameMap(`"cmd/compile/internal/syntax"->"golang/token"`, "syntax.Pos->token.Pos", "IsKnown->IsValid")
	ast.Inspect(f, func(n ast.Node) bool {
		switch n := n.(type) {
		case *ast.ImportSpec:
			// rewrite import path "cmd/compile/internal/syntax" to "golang/token"
			if n.Path.Kind != token.STRING {
				panic("invalid import path")
			}
			m.rename(&n.Path.Value)
...
// TODO(gri) can we avoid this and just use atPos consistently in golang/types and types2?
...
// (in types2 we use an array for efficiency, in golang/types it's a slice and we
// cannot change that).
