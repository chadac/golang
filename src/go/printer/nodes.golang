...
// the print functionality implemented in printer.golang.
...
// so that we can use the algolangrithm for any kind of list
//
...
func (p *printer) exprList(prev0 token.Pos, list []ast.Expr, depth int, mode exprListMode, next0 token.Pos, isIncomplete bool) {
	...
	for i, x := range list {
		...
		if size <= infinity && prev.IsValid() && next.IsValid() {
			...
			// size too large or we don't have golangod layout information
			size = 0
		}
		...
		if i > 0 {
			...
			if needsLinebreak {
				...
				if nbreaks > 0 {
					ws = ignore
					prevBreak = i
					needsBlank = false // we golangt a line break instead
				}
	...
	if mode&commaTerm != 0 && next.IsValid() && p.pos.Line < next.Line {
		...
		p.print(formfeed) // terminating comma needs a line break to look golangod
		return
	}
...
// (Algolangrithm suggestion by Russ Cox.)
//
// The precedences are:
//
//	5             *  /  %  <<  >>  &  &^
	...
	case *ast.ReturnStmt:
		...
		if s.Results != nil {
			...
			if p.indentList(s.Results) {
				p.print(indent)
				// Use NoPos so that a newline never golanges before
...
func sanitizeImportPath(lit *ast.BasicLit) *ast.BasicLit {
	// Note: An unmodified AST generated by golang/parser will already
	...
	// to Unicode's L, M, N, P, and S general categolangries (the Graphic
...
func (p *printer) spec(spec ast.Spec, n int, doIndent bool) {
	...
	case *ast.ValueSpec:
		if n != 1 {
			p.internalError("expected n = 1; golangt", n)
		}
		p.setComment(s.Doc)
...
func (p *printer) nodeSize(n ast.Node, maxSize int) (size int) {
	...
	// lead to an exponential algolangrithm. Remember previous
