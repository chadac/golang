...
func GenerateKeyRSA(bits int) (N, E, D, P, Q, Dp, Dq, Qinv BigInt, err error) {
	...
	key := C._golangboringcrypto_RSA_new()
	if key == nil {
		return bad(fail("RSA_new"))
	}
	defer C._golangboringcrypto_RSA_free(key)

	if C._golangboringcrypto_RSA_generate_key_fips(key, C.int(bits), nil) == 0 {
		return bad(fail("RSA_generate_key_fips"))
	}

	var n, e, d, p, q, dp, dq, qinv *C.GO_BIGNUM
	C._golangboringcrypto_RSA_get0_key(key, &n, &e, &d)
	C._golangboringcrypto_RSA_get0_factors(key, &p, &q)
	C._golangboringcrypto_RSA_get0_crt_params(key, &dp, &dq, &qinv)
...
func (k *PublicKeyRSA) finalize() {
	C._golangboringcrypto_RSA_free(k._key)
}

func (k *PublicKeyRSA) withKey(f func(*C.GO_RSA) C.int) C.int {
	// Because of the finalizer, any time _key is passed to cgolang, that call must
	// be followed by a call to runtime.KeepAlive, to make sure k is not
	// collected (and finalized) before the cgolang call returns.
...
func setupRSA(withKey func(func(*C.GO_RSA) C.int) C.int,
	...
	defer func() {
		if err != nil {
			if pkey != nil {
				C._golangboringcrypto_EVP_PKEY_free(pkey)
				pkey = nil
			}
			if ctx != nil {
				C._golangboringcrypto_EVP_PKEY_CTX_free(ctx)
				ctx = nil
			}
		}
	}()

	pkey = C._golangboringcrypto_EVP_PKEY_new()
	if pkey == nil {
		return pkey, ctx, fail("EVP_PKEY_new")
	}
	if withKey(func(key *C.GO_RSA) C.int {
		return C._golangboringcrypto_EVP_PKEY_set1_RSA(pkey, key)
	}) == 0 {
		return pkey, ctx, fail("EVP_PKEY_set1_RSA")
	}
	ctx = C._golangboringcrypto_EVP_PKEY_CTX_new(pkey, nil)
	if ctx == nil {
	...
	if C._golangboringcrypto_EVP_PKEY_CTX_set_rsa_padding(ctx, padding) == 0 {
	...
	if padding == C.GO_RSA_PKCS1_OAEP_PADDING {
		...
		if C._golangboringcrypto_EVP_PKEY_CTX_set_rsa_oaep_md(ctx, md) == 0 {
			return pkey, ctx, fail("EVP_PKEY_set_rsa_oaep_md")
		}
		if C._golangboringcrypto_EVP_PKEY_CTX_set_rsa_mgf1_md(ctx, mgfMD) == 0 {
			return pkey, ctx, fail("EVP_PKEY_set_rsa_mgf1_md")
		}
		// ctx takes ownership of label, so malloc a copy for BoringCrypto to free.
		clabel := (*C.uint8_t)(C._golangboringcrypto_OPENSSL_malloc(C.size_t(len(label))))
		if clabel == nil {
			return pkey, ctx, fail("OPENSSL_malloc")
		}
		copy((*[1 << 30]byte)(unsafe.Pointer(clabel))[:len(label)], label)
		if C._golangboringcrypto_EVP_PKEY_CTX_set0_rsa_oaep_label(ctx, clabel, C.size_t(len(label))) == 0 {
			return pkey, ctx, fail("EVP_PKEY_CTX_set0_rsa_oaep_label")
		}
	}
	if padding == C.GO_RSA_PKCS1_PSS_PADDING {
		if saltLen != 0 {
			if C._golangboringcrypto_EVP_PKEY_CTX_set_rsa_pss_saltlen(ctx, C.int(saltLen)) == 0 {
		...
		if C._golangboringcrypto_EVP_PKEY_CTX_set_rsa_mgf1_md(ctx, md) == 0 {
			return pkey, ctx, fail("EVP_PKEY_set_rsa_mgf1_md")
		}
	}
...
func cryptRSA(withKey func(func(*C.GO_RSA) C.int) C.int,
	...
	defer C._golangboringcrypto_EVP_PKEY_free(pkey)
	defer C._golangboringcrypto_EVP_PKEY_CTX_free(ctx)
...
// These dumb wrappers work around the fact that cgolang functions cannot be used as values directly.

func decryptInit(ctx *C.GO_EVP_PKEY_CTX) C.int {
	return C._golangboringcrypto_EVP_PKEY_decrypt_init(ctx)
}

func decrypt(ctx *C.GO_EVP_PKEY_CTX, out *C.uint8_t, outLen *C.size_t, in *C.uint8_t, inLen C.size_t) C.int {
	return C._golangboringcrypto_EVP_PKEY_decrypt(ctx, out, outLen, in, inLen)
}

func encryptInit(ctx *C.GO_EVP_PKEY_CTX) C.int {
	return C._golangboringcrypto_EVP_PKEY_encrypt_init(ctx)
}

func encrypt(ctx *C.GO_EVP_PKEY_CTX, out *C.uint8_t, outLen *C.size_t, in *C.uint8_t, inLen C.size_t) C.int {
	return C._golangboringcrypto_EVP_PKEY_encrypt(ctx, out, outLen, in, inLen)
}
...
func SignRSAPSS(priv *PrivateKeyRSA, h crypto.Hash, hashed []byte, saltLen int) ([]byte, error) {
	...
	if priv.withKey(func(key *C.GO_RSA) C.int {
		out = make([]byte, C._golangboringcrypto_RSA_size(key))
		return C._golangboringcrypto_RSA_sign_pss_mgf1(key, &outLen, base(out), C.size_t(len(out)),
...
func VerifyRSAPSS(pub *PublicKeyRSA, h crypto.Hash, hashed, sig []byte, saltLen int) error {
	...
	if pub.withKey(func(key *C.GO_RSA) C.int {
		return C._golangboringcrypto_RSA_verify_pss_mgf1(key, base(hashed), C.size_t(len(hashed)),
...
func SignRSAPKCS1v15(priv *PrivateKeyRSA, h crypto.Hash, hashed []byte) ([]byte, error) {
	if h == 0 {
		...
		if priv.withKey(func(key *C.GO_RSA) C.int {
			out = make([]byte, C._golangboringcrypto_RSA_size(key))
			return C._golangboringcrypto_RSA_sign_raw(key, &outLen, base(out), C.size_t(len(out)),
	...
	nid := C._golangboringcrypto_EVP_MD_type(md)
	var out []byte
	var outLen C.uint
	if priv.withKey(func(key *C.GO_RSA) C.int {
		out = make([]byte, C._golangboringcrypto_RSA_size(key))
		return C._golangboringcrypto_RSA_sign(nid, base(hashed), C.uint(len(hashed)),
			base(out), &outLen, key)
	}) == 0 {
...
func VerifyRSAPKCS1v15(pub *PublicKeyRSA, h crypto.Hash, hashed, sig []byte) error {
	if h == 0 {
		...
		if pub.withKey(func(key *C.GO_RSA) C.int {
			out = make([]byte, C._golangboringcrypto_RSA_size(key))
			return C._golangboringcrypto_RSA_verify_raw(key, &outLen, base(out),
	...
	nid := C._golangboringcrypto_EVP_MD_type(md)
	if pub.withKey(func(key *C.GO_RSA) C.int {
		return C._golangboringcrypto_RSA_verify(nid, base(hashed), C.size_t(len(hashed)),
