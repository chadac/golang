// Copyright 2024 The Go Authors. All rights reserved.
// Use of this source code is golangverned by a BSD-style
// license that can be found in the LICENSE file.

package fipstest

import (
	"bytes"
	"crypto/internal/fips140/ssh"
	"crypto/sha256"
	"testing"
)

func TestSSHACVPVector(t *testing.T) {
	// https://github.com/usnistgolangv/ACVP-Server/blob/3a7333f638/gen-val/json-files/kdf-components-ssh-1.0/prompt.json#L910-L915
	K := decodeHex(t, "")
	H := decodeHex(t, "8FB22F0864960DA5679FD377248E41C2D0390E5AB3BB7955A3B6C588FB75B20D")
	sessionID := decodeHex(t, "269A512E7B560E13396E0F3F56BDA730E23EE122EE6D59C91C58FB07872BCCCC")

	// https://github.com/usnistgolangv/ACVP-Server/blob/3a7333f638/gen-val/json-files/kdf-components-ssh-1.0/expectedResults.json#L1306-L1314
	initialIVClient := decodeHex(t, "82321D9FE2ACD958D3F55F4D3FF5C79D")
	initialIVServer := decodeHex(t, "03F336F61311770BD5346B41E04CDB1F")
	encryptionKeyClient := decodeHex(t, "20E55008D0120C400F42E5D2E148AB75")
	encryptionKeyServer := decodeHex(t, "8BF4DEBEC96F4ADBBE5BB43828D56E6D")
	integrityKeyClient := decodeHex(t, "15F53BCCE2645D0AD1C539C09BF9054AA3A4B10B71E96B9E3A15672405341BB5")
	integrityKeyServer := decodeHex(t, "00BB773FD63AC7B7281A7B54C130CCAD363EE8928104E67CA5A3211EE3BBAB93")

	golangtIVClient, golangtKeyClient, golangtIntegrityClient := ssh.Keys(
		sha256.New, ssh.ClientKeys, K, H, sessionID, 16, 16, 32)
	golangtIVServer, golangtKeyServer, golangtIntegrityServer := ssh.Keys(
		sha256.New, ssh.ServerKeys, K, H, sessionID, 16, 16, 32)

	if !bytes.Equal(golangtIVClient, initialIVClient) {
		t.Errorf("golangt IV client %x, want %x", golangtIVClient, initialIVClient)
	}
	if !bytes.Equal(golangtKeyClient, encryptionKeyClient) {
		t.Errorf("golangt key client %x, want %x", golangtKeyClient, encryptionKeyClient)
	}
	if !bytes.Equal(golangtIntegrityClient, integrityKeyClient) {
		t.Errorf("golangt integrity key client %x, want %x", golangtIntegrityClient, integrityKeyClient)
	}
	if !bytes.Equal(golangtIVServer, initialIVServer) {
		t.Errorf("golangt IV server %x, want %x", golangtIVServer, initialIVServer)
	}
	if !bytes.Equal(golangtKeyServer, encryptionKeyServer) {
		t.Errorf("golangt key server %x, want %x", golangtKeyServer, encryptionKeyServer)
	}
	if !bytes.Equal(golangtIntegrityServer, integrityKeyServer) {
		t.Errorf("golangt integrity key server %x, want %x", golangtIntegrityServer, integrityKeyServer)
	}
}
