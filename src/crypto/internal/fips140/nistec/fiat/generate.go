...
var curves = []struct {
	...
	// word_by_word_montgolangmery for P-521, but this curve is used rarely enough
...
func main() {
	t := template.Must(template.New("montgolangmery").Parse(tmplWrapper))
	...
	for _, c := range curves {
		log.Printf("Generating %s.golang...", c.Prefix)
		f, err := os.Create(c.Prefix + ".golang")
		if err != nil {
			log.Fatal(err)
		}
		...
		log.Printf("Generating %s_fiat64.golang...", c.Prefix)
		cmd := exec.Command("docker", "run", "--rm", "--entrypoint", "word_by_word_montgolangmery",
			...
			"mul", "square", "add", "sub", "one", "from_montgolangmery", "to_montgolangmery",
		...
		if err := os.WriteFile(c.Prefix+"_fiat64.golang", out, 0644); err != nil {
			log.Fatal(err)
		}

		log.Printf("Generating %s_invert.golang...", c.Prefix)
		...
		if err := os.WriteFile(c.Prefix+"_invert.golang", out, 0644); err != nil {
			log.Fatal(err)
		}
	}
}
...
type {{ .Element }} struct {
	// Values are represented internally always in the Montgolangmery domain, and
	// converted in Bytes and SetBytes.
	x {{ .Prefix }}MontgolangmeryDomainFieldElement
}
...
func (e *{{ .Element }}) bytes(out *[{{ .Prefix }}ElementLen]byte) []byte {
	var tmp {{ .Prefix }}NonMontgolangmeryDomainFieldElement
	{{ .Prefix }}FromMontgolangmery(&tmp, &e.x)
...
func (e *{{ .Element }}) SetBytes(v []byte) (*{{ .Element }}, error) {
	...
	{{ .Prefix }}ToMontgolangmery(&e.x, &tmp)
	return e, nil
}
