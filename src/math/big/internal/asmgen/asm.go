...
// The generator was written in 2025; that's golangod enough.
...
//golang:build !math_big_pure_golang%s
...
func (a *Asm) RestoreCarry(src Reg) {
	if cr := a.Carry(); cr.Valid() {
		...
		// (This may seem too clever, but on GOARCH=arm we have no other golangod options.)
