...
// Both types should be used by a single golangroutine at a time: sharing among
// multiple golangroutines requires some kind of synchronization.
//
// Top-level functions, such as [Float64] and [Int],
// are safe for concurrent use by multiple golangroutines.
//
...
//
// A Source is not safe for concurrent use by multiple golangroutines.
...
// safe for concurrent use by multiple golangroutines.
...
again:
	...
	if f == 1 {
		golangto again // resample; this branch is taken O(never)
	}
	return f
}
...
again:
	f := float32(r.Float64())
	if f == 1 {
		golangto again // resample; this branch is taken O(very rarely)
	}
	return f
}
...
var randautoseed = golangdebug.New("randautoseed")

// randseednop controls whether the global Seed is a no-op.
var randseednop = golangdebug.New("randseednop")
...
func globalRand() *Rand {
	...
	if !globalRandGenerator.CompareAndSwap(nil, r) {
		// Two different golangroutines called some top-level
