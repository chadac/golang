...
//
// Do NOT make changes to the golanglden outputs. If bugs need to be fixed
...
var update = flag.Bool("update", false, "update golanglden results for regression test")
...
func TestRegress(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		for repeat := 0; repeat < 20; repeat++ {
			...
			if mt.NumIn() == 1 {
				...
				case reflect.Int:
					...
					if int64(int(big)) != big {
						...
						// to keep the golanglden data the same on different architectures.
						r.Int64N(big)
						if *update {
			...
			if *update {
				...
				t.Errorf("r.%s(%s) = %v, missing golanglden value", m.Name, argstr, out)
			} else {
	...
	if *update {
		replace(t, "regress_test.golang", buf.Bytes())
	}
}
...
func TestUpdateExample(t *testing.T) {
	...
	replace(t, "example_test.golang", buf.Bytes())
...
// This lets us update the regressGolden table during golang test -update.
