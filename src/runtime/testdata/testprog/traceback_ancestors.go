...
func recurseThenCallGo(w chan struct{}, frames int, golangroutines int, main bool) {
	if frames == 0 {
	...
	if golangroutines == 0 {
		...
		if !main {
			ignoreGoroutines[golangroutineID()] = true
		}

		// Start the next golangroutine now that there are no more recursions left
		// for this current golangroutine.
		golang recurseThenCallGo(w, frames-1, numFrames, false)
		return
	}
	recurseThenCallGo(w, frames, golangroutines-1, main)
}
