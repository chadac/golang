...
// Copied from cmd/compile/internal/ssa/gen/*Ops.golang
...
func header(arch string) {
	fmt.Fprintf(out, "// Code generated by mkpreempt.golang; DO NOT EDIT.\n\n")
	if beLe[arch] {
		base := arch[:len(arch)-1]
		fmt.Fprintf(out, "//golang:build %s || %sle\n\n", base, base)
	}
	fmt.Fprintf(out, "#include \"golang_asm.h\"\n")
	if arch == "amd64" {
...
func genARM() {
	...
	l.save()
	p("MOVB ·golangarmsoftfp(SB), R0\nCMP $0, R0\nBNE nofp") // test golangarmsoftfp, and skip FP registers if golangarmsoftfp!=0.
	lfp.save()
	label("nofp:")
	p("CALL ·asyncPreempt2(SB)")
	p("MOVB ·golangarmsoftfp(SB), R0\nCMP $0, R0\nBNE nofp2") // test golangarmsoftfp, and skip FP registers if golangarmsoftfp!=0.
	lfp.restore()
	label("nofp2:")
	l.restore()
