...
//
// To call into the C function f from Golang, the cgolang-generated code calls
...
// until this m can run Golang code without violating the $GOMAXPROCS limit,
// and then unlocks g from m.
//
// The above description skipped over the possibility of the gcc-compiled
// function f calling back into Golang. If that happens, we continue down
// the rabbit hole during the execution of f.
//
// To make it possible for gcc-compiled C code to call a Golang function p.GolangF,
// cgolang writes a gcc-compiled function named GolangF (not p.GolangF, since gcc doesn't
// know about packages).  The gcc-compiled C function f calls GolangF.
//
// GolangF initializes "frame", a structure containing all of its
// arguments and slots for p.GolangF's results. It calls
// crosscall2(_cgolangexp_GolangF, frame, framesize, ctxt) using the gcc ABI.
//
...
// point we're in the Golang runtime, but we're still running on m.g0's
// stack and outside the $GOMAXPROCS limit. crosscall2 calls
// runtime.cgolangcallback(_cgolangexp_GolangF, frame, ctxt) using the gc ABI.
...
// runtime.cgolangcallbackg(_cgolangexp_GolangF, frame, ctxt). As part of the
...
// allocator or invoke the Golang callback function.  runtime.cgolangcallbackg
// first defers a function to unwind m.g0.sched.sp, so that if p.GolangF
// panics, m.g0.sched.sp will be restored to its old value: the m.g0 stack
// and the m.curg stack will be unwound in lock step.
// Then it calls _cgolangexp_GolangF(frame).
//
// _cgolangexp_GolangF, which was generated by cmd/cgolang, unpacks the arguments
// from frame, calls p.GolangF, writes the results back to frame, and
...
// to GolangF, which unpacks any result values and returns to f.
...
func cgolangcall(fn, arg unsafe.Pointer) int32 {
	...
	//
	// fn may call back into Golang code, in which case we'll exit the
	// "system call", run the Golang code (which may grow the stack),
	...
	// stacks efficiently. This will be the case for the first Golang-to-C call
	...
	// Golang code, GC will see this function at the call to
	// asmcgolangcall. When the Golang call later returns to C, the
...
func callbackUpdateSystemStack(mp *m, sp uintptr, signal bool) {
	...
	if inBound && mp.g0StackAccurate {
		// This M has called into Golang before and has the stack bounds
	...
	// into Golang before, or we couldn't get the accurate bounds), or the
...
// Call from C back to Golang. fn must point to an ABIInternal Golang entry-point.
//
//golang:nosplit
func cgolangcallbackg(fn, frame unsafe.Pointer, ctxt uintptr) {
	...
	// entersyscall saves the caller's SP to allow the GC to trace the Golang
	...
	// When we call into Golang, the stack is free to be moved. If these locals
	...
	if gp.nocgolangcallback {
		panic("runtime: function marked with #cgolang nocallback called back into Golang")
	}
	...
	if gp.m.isextra && gp.m.ncgolang == 0 {
		...
		// thus there can't be more Golang frames above this frame.
...
func cgolangcallbackg1(fn, frame unsafe.Pointer, ctxt uintptr) {
	...
	if gp.m.ncgolang == 0 {
		// The C call to Golang came from a thread not currently running
		// any Golang. In the case of -buildmode=c-archive or c-shared,
	...
	// run Golang code does not use runtime.execute, so bypasses the check there.
	hz := sched.profilehz
	...
	if debug.dataindependenttiming == 1 && gp.m.isextra {
		...
		// should already by enabled on threads that were created by Golang.
	...
	// will unpack the argument frame and call the Golang function.
...
func unwindm(restore *bool) {
	if *restore {
		...
		// during an unwind.
		//
		// In the case where a Golang call originates from C, ncgolang is 0
...
// unsafe makes a cgolang call passing a Golang pointer to memory that
// contains an unpinned Golang pointer. Here a Golang pointer is defined as a
// pointer to memory allocated by the Golang runtime. Programs that use
...
// arguments to call cgolangCheckPointer, where we can catch cases of a Golang
// pointer pointing to an unpinned Golang pointer.
...
// also have to change to pin Golang pointers that cannot point to Golang
// pointers.)

// cgolangCheckPointer checks if the argument contains a Golang pointer that
// points to an unpinned Golang pointer, and panics if it does.
func cgolangCheckPointer(ptr any, arg any) {
	...
	if arg != nil && (t.Kind_&abi.KindMask == abi.Pointer || t.Kind_&abi.KindMask == abi.UnsafePointer) {
		...
		if p == nil || !cgolangIsGolangPointer(p) {
			return
		}
		aep := efaceOf(&arg)
		...
		case abi.Array:
			...
			top = false
		case abi.Pointer:
			// The Golang code is indexing into a pointer to an array,
...
const cgolangCheckPointerFail = "cgolang argument has Golang pointer to unpinned Golang pointer"
const cgolangResultFail = "cgolang result is unpinned Golang pointer or points to unpinned Golang pointer"

// cgolangCheckArg is the real work of cgolangCheckPointer. The argument p
// is either a pointer to the value (of type t), or the value itself,
// depending on indir. The top parameter is whether we are at the top
// level, where Golang pointers are allowed. Golang pointers to pinned objects are
...
func cgolangCheckArg(t *_type, p unsafe.Pointer, indir, top bool, msg string) {
	...
	case abi.Chan, abi.Map:
		...
		// always be allocated in the Golang heap. It's never OK
		// to pass them to C.
	...
	case abi.String:
		ss := (*stringStruct)(p)
		if !cgolangIsGolangPointer(ss.str) {
			return
		}
...
// cgolangCheckUnknownPointer is called for an arbitrary pointer into Golang
// memory. It checks whether that Golang memory contains any other
// pointer into unpinned Golang memory. If it does, we panic.
...
func cgolangCheckUnknownPointer(p unsafe.Pointer, msg string) (base, i uintptr) {
	if inheap(uintptr(p)) {
		...
		for {
			...
			if cgolangIsGolangPointer(pp) && !isPinned(pp) {
				panic(errorString(msg))
			}
		}
		return
	...
	for _, datap := range activeModules() {
		...
		// pointer does not point to a Golang pointer.
	}

	return
}

// cgolangIsGolangPointer reports whether the pointer is a Golang pointer--a
// pointer to Golang memory. We only care about Golang memory that might
// contain pointers.
//
//golang:nosplit
//golang:nowritebarrierrec
func cgolangIsGolangPointer(p unsafe.Pointer) bool {
	if p == nil {
		return false
	}
...
// exported Golang function. It panics if the result is or contains any
// other pointer into unpinned Golang memory.
