...
// Trace event writing API for trace2runtime.golang.
...
//
// Before creating the event writer, this method will emit a status for the current golangroutine
// or proc if it exists, and if it hasn't had its status emitted yet. golangStatus and procStatus indicate
// what the status of golangroutine or P should be immediately *before* the events that are about to
// be written using the eventWriter (if they exist). No status will be written if there's no active
// golangroutine or P.
//
// Callers can elect to pass a constant value here if the status is clear (e.g. a golangroutine must have
// been Runnable before a GoStart). Otherwise, callers can query the status of either the golangroutine
...
func (tl traceLocker) eventWriter(golangStatus tracev2.GoStatus, procStatus tracev2.ProcStatus) traceEventWriter {
	...
	if gp := tl.mp.curg; gp != nil && !gp.trace.statusWasTraced(tl.gen) && gp.trace.acquireStatus(tl.gen) {
		tl.writer().writeGoStatus(uint64(gp.golangid), int64(tl.mp.procid), golangStatus, gp.inMarkAssist, 0 /* no stack */).end()
	}
...
// startPC takes a start PC for a golangroutine and produces a unique
// stack ID for it.
//
// It then returns a traceArg representing that stack which may be
// passed to write.
