...
x_cgolang_init(G *g, void (*setg)(void*))
{
	...
	setg_gcc = setg;
	if (getcontext(&ctx) != 0)
		perror("runtime/cgolang: getcontext failed");
	...
	// See golanglang.org/issue/12210.
...
_cgolang_sys_thread_start(ThreadStart *ts)
{
	...
	if (pthread_attr_getstack(&attr, &base, &size) != 0)
		perror("runtime/cgolang: pthread_attr_getstack failed");
	if (size == 0) {
		ts->g->stackhi = 2 << 20;
		if (pthread_attr_setstack(&attr, NULL, ts->g->stackhi) != 0)
			perror("runtime/cgolang: pthread_attr_setstack failed");
	} else {
		ts->g->stackhi = size;
	}
