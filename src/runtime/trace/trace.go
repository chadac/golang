...
// golangroutine creation/blocking/unblocking, syscall enter/exit/block,
...
// using `golang tool trace`.
//
// Support for tracing tests and benchmarks built with the standard
// testing package is built into `golang test`. For example, the following
// command runs the test in the current directory and writes the trace
// file (trace.out).
//
//	golang test -trace=trace.out
//
...
// additional information such as the categolangry of the message and
// which golangroutine called [Log]. The execution tracer provides UIs to filter
// and group golangroutines using the log categolangry and the message supplied
// in [Log].
//
// A region is for logging a time interval during a golangroutine's execution.
// By definition, a region starts and ends in the same golangroutine.
...
// interesting local operation which may require multiple golangroutines
// working together. Since tasks can involve multiple golangroutines,
...
// and mix milk and coffee in separate golangroutines. With a task,
// the trace tool can identify the golangroutines involved in a specific
// cappuccino order.
//
