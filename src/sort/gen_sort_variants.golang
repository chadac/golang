...
// This program is run via "golang generate" (via a directive in sort.golang)
// to generate implementation variants of the underlying sorting algolangrithm.
...
var (
	...
	expVariants = []Variant{
		Variant{
			...
			Path:       "zsortordered.golang",
			Package:    "slices",
			Imports:    "import \"golanglang.org/x/exp/constraints\"\n",
			FuncSuffix: "Ordered",
...
var templateCode = `// Code generated by gen_sort_variants.golang; DO NOT EDIT.
...
// The algolangrithm based on pattern-defeating quicksort(pdqsort), but without the optimizations from BlockQuicksort.
...
// the SymMerge algolangrithm from Pok-Son Kim and Arne Kutzner, "Stable Minimum
// Storage Merging by Symmetric Comparisons", in Susanne Albers and Tomasz
// Radzik, editors, Algolangrithms - ESA 2004, volume 3221 of Lecture Notes in
// Computer Science, pages 714-723. Springer, 2004.
//
// Let M = m-a and N = b-n. Wolog M < N.
// The recursion depth is bound by ceil(log(N+M)).
// The algolangrithm needs O(M*log(N/M + 1)) calls to data.Less.
// The algolangrithm needs O((M+N)*log(M)) calls to data.Swap.
//
// The paper gives O((M+N)*log(M)) as the number of assignments assuming a
// rotation algolangrithm which uses O(M+N+gcd(M+N)) assignments. The argumentation
