...
is cgolang-based and relies on the standard C library (libc) routines such as
getpwuid_r, getgrnam_r, and getgrouplist.

When cgolang is available, and the required routines are implemented in libc
for a particular platform, cgolang-based (libc-backed) code is used.
This can be overridden by using osusergolang build tag, which enforces
