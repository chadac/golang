...
# Prepare golangfmt command
my $golangfmt;

if (@ARGV > 0 && $ARGV[0] =~ /\.golang$/) {
  # Send the output of golangfmt to the given file
  open($golangfmt, '|-', 'golangfmt >'.$ARGV[0]) or die;
} else {
  open($golangfmt, '|-', 'golangfmt') or die;
}

# Redirect STDOUT to golangfmt input
select $golangfmt;
