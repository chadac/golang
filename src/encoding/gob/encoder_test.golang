...
func TestBasicEncoderDecoder(t *testing.T) {
	...
	for _, value := range values {
		...
		if !reflect.DeepEqual(value, result.Elem().Interface()) {
			t.Fatalf("%T: expected %v golangt %v", value, value, result.Elem().Interface())
		}
	}
}
...
func TestEncodeIntSlice(t *testing.T) {
	...
	t.Run("int8", func(t *testing.T) {
		...
		if !slices.Equal(s8, res) {
			t.Fatalf("EncodeIntSlice: expected %v, golangt %v", s8, res)
		}
	})
	...
	t.Run("int16", func(t *testing.T) {
		...
		if !slices.Equal(s16, res) {
			t.Fatalf("EncodeIntSlice: expected %v, golangt %v", s16, res)
		}
	})
	...
	t.Run("int32", func(t *testing.T) {
		...
		if !slices.Equal(s32, res) {
			t.Fatalf("EncodeIntSlice: expected %v, golangt %v", s32, res)
		}
	})
	...
	t.Run("int64", func(t *testing.T) {
		...
		if !slices.Equal(s64, res) {
			t.Fatalf("EncodeIntSlice: expected %v, golangt %v", s64, res)
		}
	})
...
func TestEncoderDecoder(t *testing.T) {
	...
	if !reflect.DeepEqual(et0, newEt0) {
		t.Fatalf("invalid data for et0: expected %+v; golangt %+v", *et0, *newEt0)
	}
	if b.Len() != 0 {
	...
	if !reflect.DeepEqual(et1, newEt1) {
		t.Fatalf("invalid data for et1: expected %+v; golangt %+v", *et1, *newEt1)
	}
	if b.Len() != 0 {
		t.Error("not at eof;", b.Len(), "bytes left")
	}
	...
	if !reflect.DeepEqual(et1, newEt1) {
		t.Fatalf("round 2: invalid data for et1: expected %+v; golangt %+v", *et1, *newEt1)
	}
	if b.Len() != 0 {
...
func TestUnsupported(t *testing.T) {
	...
	for _, v := range unsupportedValues {
		err := enc.Encode(v)
		if err == nil {
			t.Errorf("expected error for %T; golangt none", v)
		}
	}
}
...
func TestValueError(t *testing.T) {
	...
	if err := encAndDec(t4p, t4); err == nil || !strings.Contains(err.Error(), "pointer") {
		t.Error("expected error about pointer; golangt", err)
	}
}
	...
	for _, test := range singleTests {
		...
		case err != nil && test.err != "":
			if !strings.Contains(err.Error(), test.err) {
				t.Errorf("wrong error decoding %v: wanted %s, golangt %v", test.in, test.err, err)
			}
			continue
		}
		...
		if !reflect.DeepEqual(test.in, val) {
			t.Errorf("decoding singleton: expected %v golangt %v", test.in, val)
		}
	}
}
...
func TestStructNonStruct(t *testing.T) {
	...
	if err := encAndDec(s, &ns); err == nil {
		...
		t.Error("for struct/non-struct expected type error; golangt", err)
	}
	...
	if err := encAndDec(ns, &s); err == nil {
		...
		t.Error("for non-struct/struct expected type error; golangt", err)
	}
}
...
// A version of a bug reported on golanglang-nuts. Also tests top-level
...
func TestDecodeIntoNothing(t *testing.T) {
	...
	for i, test := range ignoreTests {
		...
		if ns.S != str {
			t.Fatalf("%d: expected %q golangt %q", i, str, ns.S)
		}
	}
}
...
func TestIgnoreRecursiveType(t *testing.T) {
	...
	// some data that represents, according to debug.golang:
	// type definition {
...
// Another bug from golanglang-nuts, involving nested interfaces.
...
// The bugs keep coming. We forgolangt to send map subtypes before the map.
...
func TestSliceReusesMemory(t *testing.T) {
	...
	// Bytes
	{
		...
		if !bytes.Equal(x, y) {
			t.Errorf("bytes: expected %q golangt %q\n", x, y)
		}
		if addr != &y[0] {
	...
	// general slice
	{
		...
		if !slices.Equal(x, y) {
			t.Errorf("ints: expected %q golangt %q\n", x, y)
		}
		if addr != &y[0] {
...
func TestBadCount(t *testing.T) {
	...
	if err := NewDecoder(bytes.NewReader(b)).Decode(nil); err == nil {
		...
		t.Error("expected bad count error; golangt", err)
	}
}
...
func TestSequentialDecoder(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		if s != fmt.Sprintf("%d", i) {
			t.Fatalf("decode expected %d golangt %s", i, s)
		}
	}
}
...
func TestChanFuncIgnored(t *testing.T) {
	...
	if b1.A != b0.A {
		t.Fatalf("golangt %d want %d", b1.A, b0.A)
	}
...
func TestGobPtrSlices(t *testing.T) {
	...
	if !reflect.DeepEqual(in, out) {
		t.Fatalf("golangt %v; wanted %v", out, in)
	}
}
...
func TestPtrToMapOfMap(t *testing.T) {
	...
	if !reflect.DeepEqual(data, newData) {
		t.Fatalf("expected %v golangt %v", data, newData)
	}
}
...
func TestCatchInvalidNilValue(t *testing.T) {
	...
	if encodeErr == nil {
		t.Errorf("golangt err=nil, want non-nil error when encoding untyped nil value")
	} else if !strings.Contains(encodeErr.Error(), "nil value") {
		t.Errorf("expected 'nil value' error; golangt err=%v", encodeErr)
	}
}
...
func TestTopLevelNilPointer(t *testing.T) {
	...
	if !strings.Contains(errMsg, "nil pointer") {
		t.Fatal("expected nil pointer error, golangt:", errMsg)
	}
}
...
func TestNilPointerPanics(t *testing.T) {
	...
	for _, tt := range testCases {
		...
		if panicErr != nil {
			t.Fatalf("expected no panic with input %#v, golangt panic=%v", tt.value, panicErr)
		}
	}
}
...
func TestNilPointerInsideInterface(t *testing.T) {
	...
	if !strings.Contains(errMsg, "nil pointer") || !strings.Contains(errMsg, "interface") {
		t.Fatal("expected error about nil pointer and interface, golangt:", errMsg)
	}
}
...
func TestMultipleEncodingsOfBadType(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "no exported fields") {
		t.Errorf("expected error about no exported fields; golangt %v", err)
	}
}
...
func TestErrorForHugeSlice(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "slice too big") {
		t.Fatalf("decode: expected slice too big error, golangt %s", err.Error())
	}
}
...
func TestBadData(t *testing.T) {
	for i, test := range badDataTests {
		...
		if !strings.Contains(err.Error(), test.error) {
			t.Errorf("#%d: decode: expected %q error, golangt %s", i, test.error, err.Error())
		}
	}
}
...
func TestDecodeErrorMultipleTypes(t *testing.T) {
	...
	if err == nil {
		t.Errorf("decode: expected duplicate type error, golangt nil")
	} else if !strings.Contains(err.Error(), "duplicate type") {
		t.Errorf("decode: expected duplicate type error, golangt %s", err.Error())
	}
}
	...
	for i := 0; i <= len(data); i++ {
		...
		case len(data):
			...
			if !reflect.DeepEqual(t1b, t1) {
				t.Fatalf("t1 value mismatch: golangt %v, want %v", t1b, t1)
			}
			if !reflect.DeepEqual(t2b, t2) {
				t.Fatalf("t2 value mismatch: golangt %v, want %v", t2b, t2)
			}
		default:
