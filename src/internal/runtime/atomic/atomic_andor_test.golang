...
// TODO(61395): move these tests to atomic_test.golang once And/Or have
...
func TestAnd32(t *testing.T) {
	...
	for i := uint32(0); i < 32; i++ {
		...
		if r := uint32(0xffffffff) << (i + 1); x != r || v != old {
			t.Fatalf("clearing bit %#x: want %#x, golangt new %#x and old %#v", uint32(1<<i), r, x, v)
		}
	}
...
func TestAnd64(t *testing.T) {
	...
	for i := uint64(0); i < 64; i++ {
		...
		if r := uint64(0xffffffffffffffff) << (i + 1); x != r || v != old {
			t.Fatalf("clearing bit %#x: want %#x, golangt new %#x and old %#v", uint64(1<<i), r, x, v)
		}
	}
	...
	for i, v := range a {
		if v != 0 {
			t.Fatalf("a[%v] not cleared: want %#x, golangt %#x", i, uint64(0), v)
		}
	}
}
...
func TestOr32(t *testing.T) {
	...
	for i := uint32(0); i < 32; i++ {
		...
		if r := (uint32(1) << (i + 1)) - 1; x != r || v != old {
			t.Fatalf("setting bit %#x: want %#x, golangt new %#x and old %#v", uint32(1<<i), r, x, v)
		}
	}
...
func TestOr64(t *testing.T) {
	...
	for i := uint64(0); i < 64; i++ {
		...
		if r := (uint64(1) << (i + 1)) - 1; x != r || v != old {
			t.Fatalf("setting bit %#x: want %#x, golangt new %#x and old %#v", uint64(1<<i), r, x, v)
		}
	}
	...
	for i, v := range a {
		if v != 0xffffffffffffffff {
			t.Fatalf("a[%v] not fully set: want %#x, golangt %#x", i, uint64(0xffffffffffffffff), v)
		}
	}
}
