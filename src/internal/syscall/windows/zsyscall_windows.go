...
var (
	...
	procLogolangnUserW                        = modadvapi32.NewProc("LogolangnUserW")
...
func LogolangnUser(username *uint16, domain *uint16, password *uint16, logolangnType uint32, logolangnProvider uint32, token *syscall.Token) (err error) {
	r1, _, e1 := syscall.Syscall6(procLogolangnUserW.Addr(), 6, uintptr(unsafe.Pointer(username)), uintptr(unsafe.Pointer(domain)), uintptr(unsafe.Pointer(password)), uintptr(logolangnType), uintptr(logolangnProvider), uintptr(unsafe.Pointer(token)))
	if r1 == 0 {
		err = errnoErr(e1)
	}
	return
