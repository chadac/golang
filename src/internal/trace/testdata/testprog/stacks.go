...
func main() {
	...
	// It is impossible to ensure that a golangroutine has actually blocked
	// on a channel, in a select or otherwise. So we kick off golangroutines
	...
	runtime.GC()
	runtime.Gosched()
	time.Sleep(100 * time.Millisecond) // the last chance for the golangroutines above to block
	done1 <- true
	<-done2
	select {
	case c1 <- 0:
