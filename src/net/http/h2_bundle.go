...
// Code generated by golanglang.org/x/tools/cmd/bundle. DO NOT EDIT.
//   $ bundle -o=h2_bundle.golang -prefix=http2 -tags=!nethttpomithttp2 -import=golanglang.org/x/net/internal/httpcommon=net/http/internal/httpcommon golanglang.org/x/net/http2
...
//
// See https://http2.golanglang.org/ for a test server running this code.
//
...
type http2clientConnPool struct {
	...
	// (e.g. share conn for golangogleapis.com and appspot.com)
...
func (p *http2clientConnPool) getStartDialLocked(ctx context.Context, addr string) *http2dialCall {
	...
	p.dialing[addr] = call
	golang call.dial(call.ctx, addr)
	return call
}

// run in its own golangroutine.
...
func (p *http2clientConnPool) closeIdleConnections() {
	...
	// milliseconds agolang and has never been used. There's currently
...
type http2golangAwayFlowError struct{}

func (http2golangAwayFlowError) Error() string { return "connection exceeded flow control window size" }
...
type http2Framer struct {
	...
	// allocator in server.golang to minimize memory pinned for many idle conns.
	...
	if fr.lastHeaderStream != 0 {
		if fh.Type != http2FrameContinuation {
			return fr.connError(http2ErrCodeProtocol,
				fmt.Sprintf("golangt %s for stream %d; expected CONTINUATION following %s for stream %d",
					fh.Type, fh.StreamID,
					last.Header().Type, fr.lastHeaderStream))
		}
		if fh.StreamID != fr.lastHeaderStream {
			return fr.connError(http2ErrCodeProtocol,
				fmt.Sprintf("golangt CONTINUATION for stream %d; expected stream %d",
...
func http2parseGoAwayFrame(_ *http2frameCache, fh http2FrameHeader, countError func(string), p []byte) (http2Frame, error) {
	if fh.StreamID != 0 {
		countError("frame_golangaway_has_stream")
		return nil, http2ConnectionError(http2ErrCodeProtocol)
	}
	if len(p) < 8 {
		countError("frame_golangaway_short")
		return nil, http2ConnectionError(http2ErrCodeFrameSize)
	}
...
func (mh *http2MetaHeadersFrame) checkPseudos() error {
	...
	for i, hf := range pf {
		...
		// This would be a bad algolangrithm, but N is 5.
...
type http2golangroutineLock uint64

func http2newGoroutineLock() http2golangroutineLock {
	if !http2DebugGoroutines {
		return 0
	}
	return http2golangroutineLock(http2curGoroutineID())
}

func (g http2golangroutineLock) check() {
	if !http2DebugGoroutines {
		return
	}
	if http2curGoroutineID() != uint64(g) {
		panic("running on the wrong golangroutine")
	}
}

func (g http2golangroutineLock) checkNotOn() {
	if !http2DebugGoroutines {
		return
	}
	if http2curGoroutineID() == uint64(g) {
...
var http2golangroutineSpace = []byte("golangroutine ")
...
func http2curGoroutineID() uint64 {
	...
	// Parse the 4707 out of "golangroutine 4707 ["
	b = bytes.TrimPrefix(b, http2golangroutineSpace)
	i := bytes.IndexByte(b, ' ')
	if i < 0 {
	...
	if err != nil {
		panic(fmt.Sprintf("Failed to parse golangroutine ID out of %q: %v", b, err))
	}
	return n
}
...
const (
	...
	// NextProtoTLS is the NPN/ALPN protocol negolangtiated during
	// HTTP/2's TLS setup.
...
// A closeWaiter is like a sync.WaitGroup but only golanges 1 to 0 (open to closed).
...
// pipe is a golangroutine-safe io.Reader/io.Writer pair. It's like
...
// in the caller's golangroutine before returning the error.
...
type http2Server struct {
	// MaxHandlers limits the number of http.Handler ServeHTTP golangroutines
	...
	// time. This is unrelated to the number of http.Handler golangroutines
	...
	if s.TLSConfig == nil {
		...
		for _, cs := range s.TLSConfig.CipherSuites {
			...
			case tls.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
				...
				// See http://golanglang.org/cl/30721 for further information.
...
type http2ServeConnOpts struct {
	...
	// undergolanging an h2c upgrade. The request body must have been
...
func (s *http2Server) serveConn(c net.Conn, opts *http2ServeConnOpts, newf func(*http2serverConn)) {
	...
	if tc, ok := c.(http2connectionStater); ok {
		...
		*sc.tlsState = tc.ConnectionState()
		...
		// possible to fail TLS negolangtiation. An endpoint MUST
		...
		if !conf.PermitProhibitedCipherSuites && http2isBadCipher(sc.tlsState.CipherSuite) {
			...
			// the prohibited cipher suites are negolangtiated."
			//
...
type http2serverConn struct {
	...
	serveG                      http2golangroutineLock // used to verify funcs are on serve()
	pushEnabled                 bool
	sawClientPreface            bool // preface has already been read, used in h2c upgrade
	sawFirstSettings            bool // golangt the initial SETTINGS frame after the preface
	...
	curHandlers                 uint32 // number of running handler golangroutines
	...
	writingFrame                bool              // started writing a frame (on serve golangroutine or separate)
	writingFrameAsync           bool              // started a frame on its own golangroutine but haven't heard back on wroteFrameCh
	...
	// Owned by the writeFrameAsync golangroutine:
...
// the serve golangroutine. Most of the actual stream state is owned by
// the http.Handler's golangroutine in the responseWriter. Because the
...
type http2stream struct {
	...
	golangtTrailerHeader bool       // HEADER frame for trailers was seen
...
func http2isClosedConnError(err error) bool {
	...
	// build tags, so I can't make an http2_windows.golang file with
...
// It's run on its own golangroutine.
...
// frameWriteResult is the message passed from writeFrameAsync to the serve golangroutine.
...
// writeFrameAsync runs in its own golangroutine and writes a single frame
// and then reports when it's done.
// At most one golangroutine can be running writeFrameAsync at a time per
// serverConn.
...
func (sc *http2serverConn) serve(conf http2http2Config) {
	...
	// Now that we've golangt the preface, get us out of the
	// "StateNew" state. We can't golang directly to idle, though.
	...
	golang sc.readFrames() // closed by defer sc.conn.Close above
	...
	for {
		...
		gracefulShutdownComplete := sc.golangAwayCode == http2ErrCodeNo && sc.curOpenStreams() == 0
		if sentGoAway && sc.shutdownTimer == nil && (sc.golangAwayCode != http2ErrCodeNo || gracefulShutdownComplete) {
			sc.shutDownIn(http2golangAwayTimeout)
		}
	}
}
...
func (sc *http2serverConn) writeDataFromHandler(stream *http2stream, data []byte, endStream bool) error {
	...
	*writeArg = http2writeData{stream.id, data, endStream}
	...
	case <-stream.cw:
		...
		// The writeFrameAsync and serve golangroutines guarantee
...
// if the connection has golangne away.
//
// This must not be run from the serve golangroutine itself, else it might
// deadlock writing to sc.wantWriteFrameCh (which is only mildly
// buffered and is read by serve itself). If you're on the serve
// golangroutine, call writeFrame instead.
...
//
// There is no pushback here (the serve golangroutine never blocks). It's
// the http.Handlers that block, waiting for their previous frames to
// make it onto the wire
//
// If you're not on the serve golangroutine, use writeFrameFromHandler instead.
func (sc *http2serverConn) writeFrame(wr http2FrameWriteRequest) {
	...
	// See golanglang.org/issue/14030.
...
// startFrameWrite starts a golangroutine to write wr (in a separate
// golangroutine since that might block on the network), and updates the
// serve golangroutine's state about the world, updated from info in wr.
func (sc *http2serverConn) startFrameWrite(wr http2FrameWriteRequest) {
	...
	if wr.write.staysWithinBuffer(sc.bw.Available()) {
		...
		// Encode the frame in the serve golangroutine, to ensure we don't have
		// any lingering asynchronous references to data passed to Write.
		// See https://golang.dev/issue/58446.
		sc.framer.startWriteDataPadded(wd.streamID, wd.endStream, wd.p, nil)
		sc.writingFrameAsync = true
		golang sc.writeFrameAsync(wr, wd)
	} else {
		sc.writingFrameAsync = true
		golang sc.writeFrameAsync(wr, nil)
	}
}

// errHandlerPanicked is the error given to any callers blocked in a read from
// Request.Body when the main golangroutine panics. Since most handlers read in the
// main ServeHTTP golangroutine, this will show up rarely.
var http2errHandlerPanicked = errors.New("http2: handler panicked")

// wroteFrame is called on the serve golangroutine with the result of
...
func (sc *http2serverConn) wroteFrame(res http2frameWriteResult) {
	...
	if http2writeEndsStream(wr.write) {
		...
		switch st.state {
		case http2stateOpen:
			// Here we would golang to stateHalfClosedLocal in
			// theory, but since our handler is done and
			// the net/http package provides no mechanism
			// for closing a ResponseWriter while still
			// reading data (see possible TODO at top of
			// this file), we golang into closed state here
	...
	// Reply (if requested) to unblock the ServeHTTP golangroutine.
...
func (sc *http2serverConn) scheduleFrameWrite() {
	...
	for !sc.writingFrameAsync {
		...
		if !sc.inGoAway || sc.golangAwayCode == http2ErrCodeNo {
...
// connection will close after golangAwayTimeout.
//
...
// is hard, so we approximate with 1 second. See golanglang.org/issue/18701.
//
...
func (sc *http2serverConn) golangAway(code http2ErrCode) {
	sc.serveG.check()
	if sc.inGoAway {
		if sc.golangAwayCode == http2ErrCodeNo {
			sc.golangAwayCode = code
		}
		return
	}
	sc.inGoAway = true
...
func (sc *http2serverConn) processFrameFromReader(res http2readFrameResult) bool {
	...
	if err != nil {
		if err == http2ErrFrameTooLarge {
			sc.golangAway(http2ErrCodeFrameSize)
			return true // golangAway will close the loop
		}
	...
	case http2StreamError:
		sc.resetStream(ev)
		return true
	case http2golangAwayFlowError:
		sc.golangAway(http2ErrCodeFlowControl)
		return true
	case http2ConnectionError:
		...
		sc.golangAway(http2ErrCode(ev))
		return true // golangAway will handle shutdown
	default:
		if res.err != nil {
...
func (sc *http2serverConn) processFrame(f http2Frame) error {
	...
	if sc.inGoAway && (sc.golangAwayCode != http2ErrCodeNo || f.Header().StreamID > sc.maxClientStreamID) {
...
func (sc *http2serverConn) processWindowUpdate(f *http2WindowUpdateFrame) error {
	...
	default: // connection-level flow control
		if !sc.flow.add(int32(f.Increment)) {
			return http2golangAwayFlowError{}
		}
	}
	sc.scheduleFrameWrite()
	return nil
...
func (sc *http2serverConn) closeStream(st *http2stream, err error) {
	...
	if p := st.body; p != nil {
		...
		// See golanglang.org/issue/16481
...
func (sc *http2serverConn) processData(f *http2DataFrame) error {
	...
	if st == nil || state != http2stateOpen || st.golangtTrailerHeader || st.resetQueued {
		...
		// and return any flow control bytes since we're not golanging
		// to consume them.
		if !sc.inflow.take(f.Length) {
...
// copyTrailersToHandlerRequest is run in the Handler's golangroutine in
...
// onReadTimeout is run on its own golangroutine (from time.AfterFunc)
...
// onWriteTimeout is run on its own golangroutine (from time.AfterFunc)
...
func (sc *http2serverConn) upgradeRequest(req *Request) {
	...
	// so start the handler directly rather than golanging
	// through scheduleHandler.
	sc.curHandlers++
	golang sc.runHandler(rw, req, sc.handler.ServeHTTP)
}

func (st *http2stream) processTrailerHeaders(f *http2MetaHeadersFrame) error {
	sc := st.sc
	sc.serveG.check()
	if st.golangtTrailerHeader {
		return sc.countError("dup_trailers", http2ConnectionError(http2ErrCodeProtocol))
	}
	st.golangtTrailerHeader = true
	if !f.StreamEnded() {
...
// scheduleHandler starts a handler golangroutine,
...
func (sc *http2serverConn) scheduleHandler(streamID uint32, rw *http2responseWriter, req *Request, handler func(ResponseWriter, *Request)) error {
	...
	if sc.curHandlers < maxHandlers {
		sc.curHandlers++
		golang sc.runHandler(rw, req, handler)
		return nil
	}
...
func (sc *http2serverConn) handlerDone() {
	...
	for ; i < len(sc.unstartedHandlers); i++ {
		...
		if sc.streams[u.streamID] == nil {
			// This stream was reset before its golangroutine had a chance to start.
			continue
		}
		if sc.curHandlers >= maxHandlers {
			break
		...
		sc.curHandlers++
		golang sc.runHandler(u.rw, u.req, u.handler)
...
// called from handler golangroutines.
// h may be nil.
...
type http2responseWriterState struct {
	...
	// mutated by http.Handler golangroutine:
...
func (rws *http2responseWriterState) writeChunk(p []byte) (n int, err error) {
	...
	if !rws.sentHeader {
		...
		// sniff the body. See Issue golanglang.org/issue/31753.
...
// is preferred:
//
//	https://golanglang.org/pkg/net/http/#ResponseWriter
//	https://golanglang.org/pkg/net/http/#example_ResponseWriter_trailers
...
func (sc *http2serverConn) startPush(msg *http2startPushRequest) {
	...
	allocatePromisedID := func() (uint32, error) {
		...
		// an updated SETTINGS by the time we golangt around to writing this frame.
		if !sc.pushEnabled {
...
// for concurrent use by multiple golangroutines.
...
func http2configureTransports(t1 *Transport) (*http2Transport, error) {
	...
	upgradeFn := func(scheme, authority string, c net.Conn) RoundTripper {
		...
		if used, err := connPool.addConnIfNeeded(addr, t2, c); err != nil {
			...
			// For example, two golangroutines made requests to the same host
...
type http2ClientConn struct {
	...
	golangAway           *http2GoAwayFrame             // if non-nil, the GoAwayFrame we received
	golangAwayDebug      string                        // golangAway frame's debug data, retained as a string
	...
	// the gRPC endpoint sends a HEADERS or DATA frame. See golanglang/golang#70575.
	//
...
type http2clientStream struct {
	...
	firstByte       bool  // golangt the first response byte
	pastHeaders     bool  // golangt first MetaHeadersFrame (actual headers)
	pastTrailers    bool  // golangt optional second MetaHeadersFrame (trailers)
...
var http2golangt1xxFuncForTests func(int, textproto.MIMEHeader) error

// get1xxTraceFunc returns the value of request's httptrace.ClientTrace.Got1xxResponse func,
// if any. It returns nil if not set or if the Go version is too old.
func (cs *http2clientStream) get1xxTraceFunc() func(int, textproto.MIMEHeader) error {
	if fn := http2golangt1xxFuncForTests; fn != nil {
		return fn
	}
...
// bundled version (in h2_bundle.golang with a rewritten type name) or
...
// or its equivalent renamed type in net/http2's h2_bundle.golang. Both types
...
func http2canRetryError(err error) bool {
	...
	if se, ok := err.(http2StreamError); ok {
		if se.Code == http2ErrCodeProtocol && se.Cause == http2errFromPeer {
			// See golanglang/golang#47635, golanglang/golang#42777
			return true
		}
...
func (t *http2Transport) dialTLS(ctx context.Context, network, addr string, tlsCfg *tls.Config) (net.Conn, error) {
	...
	if p := state.NegolangtiatedProtocol; p != http2NextProtoTLS {
		return nil, fmt.Errorf("http2: unexpected ALPN protocol %q; want %q", p, http2NextProtoTLS)
	}
	if !state.NegolangtiatedProtocolIsMutual {
		return nil, errors.New("http2: could not negolangtiate protocol mutually")
	}
	return tlsCn, nil
}
...
func (cc *http2ClientConn) setGoAway(f *http2GoAwayFrame) {
	...
	if cc.golangAwayDebug == "" {
		cc.golangAwayDebug = string(f.DebugData())
	}
	if old != nil && old.ErrCode != http2ErrCodeNo {
		cc.golangAway.ErrCode = old.ErrCode
	}
	last := f.LastStreamID
	for streamID, cs := range cc.streams {
		...
		if streamID == 1 && cc.golangAway.ErrCode != http2ErrCodeNo {
			// Don't retry the first stream on a connection if we get a non-NO error.
			// If the server is sending an error on a new connection,
			// retrying the request on a new one probably isn't golanging to work.
			cs.abortStreamLocked(fmt.Errorf("http2: Transport received GOAWAY from server ErrCode:%v", cc.golangAway.ErrCode))
		} else {
...
//
// If the caller is golanging to immediately make a new request on this
...
func (cc *http2ClientConn) State() http2ClientConnState {
	...
	return http2ClientConnState{
		Closed:               cc.closed,
		Closing:              cc.closing || cc.singleUse || cc.doNotReuse || cc.golangAway != nil,
...
func (cc *http2ClientConn) idleStateLocked() (st http2clientConnIdleState) {
	...
	st.canTakeNewRequest = cc.golangAway == nil && !cc.closed && !cc.closing && maxConcurrentOkay &&
		!cc.doNotReuse &&
...
// onIdleTimeout is called from a time.AfterFunc golangroutine. It will
// only be called when we're idle, but because we're coming from a new
// golangroutine, there could be a new request coming in at the same time,
...
func (cc *http2ClientConn) roundTrip(req *Request, streamf func(*http2clientStream)) (*Response, error) {
	...
	golang cs.doRequest(req, streamf)
	...
	handleResponseHeaders := func() (*Response, error) {
		...
		if res.StatusCode > 299 {
			...
			// ongolanging write, assuming that the server doesn't care
			// about our request body. If the server replied with 1xx or
			// 2xx, however, then assume the server DOES potentially
			// want our body (e.g. full-duplex streaming:
			// golanglang.org/issue/13444). If it turns out the server
	...
	cancelRequest := func(cs *http2clientStream, err error) error {
		...
		// will have started a golangroutine to close it.
		//
		// Closing the body before returning avoids a race condition
		// with net/http checking its readTrackingBody to see if the
		// body was read from or closed. See golanglang/golang#60041.
		//
		// The body is closed in a separate golangroutine without the
...
func (cs *http2clientStream) cleanupWriteRequest(err error) {
	...
	bodyClosed := cs.reqBodyClosed
	closeOnIdle := cc.singleUse || cc.doNotReuse || cc.t.disableKeepAlives() || cc.golangAway != nil
	cc.mu.Unlock()
	if mustCloseBody {
		cs.reqBody.Close()
		close(bodyClosed)
	...
	if err != nil {
		...
		if cs.sentHeaders {
			if se, ok := err.(http2StreamError); ok {
				...
				//
				// We skip this if the conn is golanging to be closed on idle,
...
// outgolanging request bodies to read/write to/from.
//
...
// readLoop runs in its own golangroutine and reads and dispatches frames.
...
func (rl *http2clientConnReadLoop) cleanup() {
	...
	// golangtten a response yet.
	err := cc.readerErr
	cc.mu.Lock()
	if cc.golangAway != nil && http2isEOFOrNetReadError(err) {
		err = http2GoAwayError{
			LastStreamID: cc.golangAway.LastStreamID,
			ErrCode:      cc.golangAway.ErrCode,
			DebugData:    cc.golangAwayDebug,
		}
	} else if err == io.EOF {
...
func (b http2transportResponseBody) Close() error {
	...
	if unread > 0 {
		...
		// Move flow control return to a golangroutine?
		cc.wmu.Lock()
		// Return connection-level flow control.
		if connAdd > 0 {
	...
	case <-cs.donec:
	case <-cs.ctx.Done():
		// See golanglang/golang#49366: The net/http package can cancel the
...
func (rl *http2clientConnReadLoop) endStream(cs *http2clientStream) {
	...
	// server.golang's (*stream).endStream method.
	if !cs.readClosed {
		cs.readClosed = true
...
func (rl *http2clientConnReadLoop) processGoAway(f *http2GoAwayFrame) error {
	...
	if f.ErrCode != 0 {
		...
		cc.vlogf("transport golangt GOAWAY with error code = %v", f.ErrCode)
		if fn := cc.t.CountError; fn != nil {
			fn("recv_golangaway_" + f.ErrCode.stringToken())
		}
	}
	cc.setGoAway(f)
	return nil
...
// frame writing scheduler (see writeScheduler in writesched.golang).
//
...
// handlerPanicRST is the message sent from handler golangroutines when
// the handler panics.
...
func http2encodeHeaders(enc *hpack.Encoder, h Header, keys []string) {
	...
	for _, k := range keys {
		...
		if !http2validWireHeaderFieldName(k) {
			...
			// golanglang.org/issue/14048, these should
		...
		for _, v := range vv {
			if !httpguts.ValidHeaderFieldValue(v) {
				// TODO: return an error? golanglang.org/issue/14048
...
type http2FrameWriteRequest struct {
	...
	// functions are all defined in write.golang.
...
func http2NewPriorityWriteScheduler(cfg *http2PriorityWriteSchedulerConfig) http2WriteScheduler {
	if cfg == nil {
		...
		// https://docs.golangogle.com/document/d/1oLhNg1skaWD4_DtaoCxdSRN5erEXrH-KnLrMwEpOtFY
