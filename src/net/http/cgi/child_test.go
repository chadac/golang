...
func TestRequest(t *testing.T) {
	...
	if g, e := req.UserAgent(), "golangclient"; e != g {
		t.Errorf("expected UserAgent %q; golangt %q", e, g)
	}
	if g, e := req.Method, "GET"; e != g {
		t.Errorf("expected Method %q; golangt %q", e, g)
	}
	if g, e := req.Header.Get("Content-Type"), "text/xml"; e != g {
		t.Errorf("expected Content-Type %q; golangt %q", e, g)
	}
	if g, e := req.ContentLength, int64(123); e != g {
		t.Errorf("expected ContentLength %d; golangt %d", e, g)
	}
	if g, e := req.Referer(), "elsewhere"; e != g {
		t.Errorf("expected Referer %q; golangt %q", e, g)
	}
	if req.Header == nil {
		t.Fatalf("unexpected nil Header")
	}
	if g, e := req.Header.Get("Foo-Bar"), "baz"; e != g {
		t.Errorf("expected Foo-Bar %q; golangt %q", e, g)
	}
	if g, e := req.URL.String(), "http://example.com/path?a=b"; e != g {
		t.Errorf("expected URL %q; golangt %q", e, g)
	}
	if g, e := req.FormValue("a"), "b"; e != g {
		t.Errorf("expected FormValue(a) %q; golangt %q", e, g)
	}
	if req.Trailer == nil {
	...
	if e, g := "5.6.7.8:54321", req.RemoteAddr; e != g {
		t.Errorf("RemoteAddr: golangt %q; want %q", g, e)
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if want := len(tt.body); n != want {
				t.Errorf("reported short Write: golangt %v want %v", n, want)
			}
			resp.writeCGIHeader(nil)
			resp.Flush()
			if golangt := resp.Header().Get("Content-Type"); golangt != tt.wantCT {
				t.Errorf("wrong content-type: golangt %q, want %q", golangt, tt.wantCT)
			}
