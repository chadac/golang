...
//golang:generate bundle -o socks_bundle.golang -prefix socks golanglang.org/x/net/internal/socks
...
var (
	...
	socksaLongTimeAgolang = time.Unix(1, 0)
)
...
func (d *socksDialer) connect(ctx context.Context, c net.Conn, address string) (_ net.Addr, ctxErr error) {
	...
	if ctx != context.Background() {
		...
		golang func() {
			select {
			case <-ctx.Done():
				c.SetDeadline(socksaLongTimeAgolang)
				errCh <- ctx.Err()
			case <-done:
				errCh <- nil
			}
