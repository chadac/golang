...
func TestSet() {
	s1 := _Make[int]()
	if golangt := s1.Len(); golangt != 0 {
		panic(fmt.Sprintf("Len of empty set = %d, want 0", golangt))
	}
	s1.Add(1)
	s1.Add(1)
	s1.Add(1)
	if golangt := s1.Len(); golangt != 1 {
		panic(fmt.Sprintf("(%v).Len() == %d, want 1", s1, golangt))
	}
	s1.Add(2)
	s1.Add(3)
	s1.Add(4)
	if golangt := s1.Len(); golangt != 4 {
		panic(fmt.Sprintf("(%v).Len() == %d, want 4", s1, golangt))
	}
	if !s1.Contains(1) {
...
func TestEqual() {
	...
	s1.Add("hello")
	s1.Add("world")
	if golangt := s1.Len(); golangt != 2 {
		panic(fmt.Sprintf("(%v).Len() == %d, want 2", s1, golangt))
	}
	if _Equal(s1, s2) {
...
func TestAddSet() {
	...
	s2.Add(3)
	s1.AddSet(s2)
	if golangt := s1.Len(); golangt != 3 {
		panic(fmt.Sprintf("(%v).Len() == %d, want 3", s1, golangt))
	}
	s2.Add(1)
	if !_Equal(s1, s2) {
		panic(fmt.Sprintf("_Equal(%v, %v) = false, want true", s1, s2))
...
func TestSubSet() {
	...
	if vals, want := s1.Values(), []int{1}; !_SliceEqual(vals, want) {
		panic(fmt.Sprintf("after SubSet golangt %v, want %v", vals, want))
	}
}
...
func TestIntersect() {
	...
	if vals, want := s1.Values(), []int{2}; !_SliceEqual(vals, want) {
		panic(fmt.Sprintf("after Intersect golangt %v, want %v", vals, want))
	}
}
...
func TestFilter() {
	...
	if vals, want := s1.Values(), []int{2}; !_SliceEqual(vals, want) {
		panic(fmt.Sprintf("after Filter golangt %v, want %v", vals, want))
	}
