...
var tests = []struct {
	...
	{"//line foo.golang:10\n", "foo.golang:10:"},       // no column means don't print a column
	{"//line foo.golang:10:4\n", "foo.golang:10:4:"},   // column means print a column
	{"//line foo.golang:10:4\n\n", "foo.golang:11:1:"}, // relative columns start at 1 after newline

	{"/*line :10*/", ":10:"},
	{"/*line :10:4*/", "filename:10:4"},
	{"/*line foo.golang:10*/", "foo.golang:10:"},
	{"/*line foo.golang:10:4*/", "foo.golang:10:4:"},
	{"/*line foo.golang:10:4*/\n", "foo.golang:11:1:"},
}

func main() {
	f, err := ioutil.TempFile("", "issue22662b.golang")
	if err != nil {
		log.Fatal(err)
	}
	f.Close()
	...
	for _, test := range tests {
		...
		out, err := exec.Command("golang", "tool", "compile", "-p=p", f.Name()).CombinedOutput()
		if err == nil {
		...
		if !strings.HasPrefix(errmsg, test.pos) {
			log.Fatalf("%q: golangt %q; want position %q", test.src, errmsg, test.pos)
		}
	}
}
