...
func f() int {
	...
	if !strings.Contains(s, "a.golang:15") {
		panic("missing a.golang:15")
	}
	if !strings.Contains(s, "a.golang:19") {
		panic("missing a.golang:19")
	}
	...
	iter := runtime.CallersFrames(pcs[:n])
	f, more := iter.Next()
	if f.Function != "test/a.f" || !strings.HasSuffix(f.File, "a.golang") || f.Line != 22 {
	...
	f, more = iter.Next()
	if f.Function != "test/a.init" || !strings.HasSuffix(f.File, "a.golang") || f.Line != 15 {
